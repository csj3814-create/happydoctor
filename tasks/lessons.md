...
## 2026-09-14 지우기 전에 세어보기
- **교훈**: "이 섹션 필요 없지 않나"에 답하기 전에 **실제 데이터를 센다.** 화면에 고정 문구만 보인다고 전부 그런 것이 아니었다. 122건 중 64건에는 자동 트리아지 시절의 진짜 예진 노트가 들어 있었고, 섹션을 지웠다면 그 기록이 사라졌을 것이다. 최근 몇 건의 모양이 전체의 모양은 아니다.
- **교훈**: 화면에서 무언가를 없앨 때는 **섹션이 아니라 원인을 없앤다.** 고정 안내문을 `doctorChart`로 저장하지 않으니 기존 렌더링 가드가 알아서 숨겼다. 프런트에 "이 문자열이면 숨김" 같은 비교를 심었다면 백엔드 문구가 바뀔 때 조용히 되살아났을 것이다.
- **교훈**: 상수인지 판단하는 지식은 **그 상수가 정의된 곳에 둔다.** 접수 안내는 환자 화면이 써야 해서 저장을 멈출 수 없었고, 그래서 포털 라우트에서 걸렀다. 프런트로 문자열을 복사했다면 두 곳이 따로 놀았을 것이다.
- **교훈**: 같은 결함이 한 화면에 여러 번 있을 수 있다. 보듬이 카드(저장은 되는데 화면이 안 읽음)를 고친 직후, 알림 동의 이메일에서 **똑같은 형태**가 나왔다. 타입 정의에 필드가 빠지면 그 데이터는 조용히 버려진다. 백엔드가 저장하는 필드 목록과 프런트 타입을 한 번은 대조한다.

## 2026-09-14 계산해 놓고 버리지 않기
- **교훈**: 백엔드가 이미 계산한 사실을 프런트가 버리고 있지 않은지 확인한다. `notifiedChannels`는 정확히 계산돼 응답에 실려 왔는데 `postReply()`의 반환 타입이 `Promise<void>`였다. 타입이 `void`인 함수는 **응답 본문을 조용히 버린다는 선언**이다. 같은 날 `aiDoctorSummary`와 알림 동의 이메일에서도 같은 형태가 나왔다.
- **교훈**: "성공"의 정의를 저장이 아니라 **도달**로 잡는다. 답변은 저장됐지만 아무 채널로도 나가지 않은 상태를 성공으로 표시하면, 의료진은 답했다고 믿고 환자는 모른다. 도달 수단이 없다는 사실 자체가 의료진이 지금 알아야 할 정보다.
- **교훈**: 접수 경로가 둘이면 **부가 기능은 한쪽에만 붙기 쉽다.** 보듬이 정리가 웹에만 있었고 카카오 33건은 빈손이었다. 공통 로직은 라우트에 복사하지 말고 공유 모듈로 뺀다.
- **교훈**: 모듈이 require 시점에 의존성을 캡처하면 **테스트 캐시가 목을 섞어버린다.** 스케줄러를 분리하자 기존 테스트가 첫 테스트의 목을 물려받아 깨졌다. 라우트를 다시 require하는 하네스라면 그 라우트가 쓰는 얇은 협력 모듈도 함께 캐시에서 지워야 한다.

## 2026-09-14 "최신"은 "관련 있음"이 아니다
- **교훈**: 사용자에게 링크를 줄 때 **그 링크가 지금 이야기와 관련 있는지**를 따진다. `getLatestConsultationTracking()`은 이름 그대로 최신 1건을 줬고, 그게 5개월 전 종료된 상담이어도 새 증상 바로 뒤에 붙었다. 시간 순서대로 읽으면 "방금 말한 게 접수됐다"로 읽힌다.
- **교훈**: 자격 검사는 **부작용보다 먼저** 한다. 링크를 안 붙이기로 해도, 판정이 늦으면 이미 오래된 상담에 새 조회 코드를 발급한 뒤다.
- **교훈**: 안내 문구가 요청하는 행동을 시스템이 실제로 받아낼 수 있는지 확인한다. "먼저 말씀해 주세요"라고 해놓고 자유 문장은 어디에도 저장하지 않았다. 문구와 처리 경로가 어긋나면 사용자는 자기가 뭘 잘못했다고 생각한다.
- **교훈**: 불편해도 정직한 문장을 남긴다. "상담 시작 후 다시 한 번 적어 주세요"는 없는 편이 매끄럽지만, 없으면 환자는 방금 한 말이 전달된 줄 안다.
- **교훈**: 느린 것과 고장난 것이 화면에서 같아 보이면 **고장으로 신고된다.** 타임아웃도 지연 안내도 없는 로딩 상태가 그랬다. 대기에 상한을 두고, 길어지면 길어지는 중이라고 말한다.

## 2026-09-14 맥락을 다시 만들지 말고 이어붙이기
- **교훈**: 후속 요청에 대응할 때 **앞선 산출물을 다시 만들지 않는다.** 추가 질문이 왔다고 초진 SOAP을 재생성하면 비용만 들고 내용은 같다. 달라진 것(새 질문)에만 작업하고, 기존 산출물은 보존한다.
- **교훈**: 새 필드로 저장할지 덮어쓸지는 **둘 다 필요한지**로 정한다. `aiDoctorSummary`를 덮어썼다면 의료진이 초진 차트를 잃었을 것이다.
- **교훈**: 맥락으로 넘길 이전 답변은 **첫 번째가 아니라 마지막**이다. 배열을 넘길 때 어느 쪽 끝이 최신인지 테스트로 고정한다.
- **교훈**: 공백 문자열은 truthy다. `if (!question)`은 `'   '`를 통과시킨다. 외부 입력을 가드할 때는 trim한 값으로 판단하고, 빈 입력 케이스를 테스트로 남긴다.

## 2026-09-14 두 목록은 대화가 아니다
- **교훈**: 시간 축을 공유하는 두 목록을 따로 렌더하면 사용자가 머릿속에서 합쳐야 한다. 환자 질문과 의료진 답변이 그랬다. **하나의 타임라인으로 합치고, 보는 사람을 한쪽에 고정**한다.
- **교훈**: 대화에서 **시스템 기록을 지우면 침묵이 설명되지 않는다.** 자동 종료나 재분석 로그를 빼면 답변 사이의 공백이 왜 생겼는지 알 수 없다. 작게, 가운데, 눈에 덜 띄게 남긴다.
- **교훈**: 같은 데이터라도 **보는 사람에 따라 어느 버전을 보여줄지 다르다.** 추가 질문의 한국어 번역본은 의료진용이고, 환자에게는 본인이 쓴 원문을 돌려줘야 한다.
- **교훈**: `command | tail`은 **파이프의 종료코드를 가린다.** 빌드 성패를 tail 출력으로 판단하면 실패가 통과로 보인다. `PIPESTATUS`를 보거나 파이프 없이 돌린다.
- **교훈**: 감시 필터는 **실제로 나오는 실패 문구**를 담아야 한다. `Failed to compile|Type error`만 보다가 `Next.js build worker exited with code: 1`을 놓쳤다. 필터를 좁게 잡으면 침묵이 성공처럼 보인다 — 이번엔 내 필터가 그 함정을 만들었다.

## 2026-09-14 같은 판단을 여러 곳에서 하지 않기
- **교훈**: 같은 판정을 여러 곳에 인라인으로 적으면 **값이 하나 추가될 때 일부만 갱신된다.** "상담이 끝났는가"가 8곳에 각각 적혀 있었고, 그래서 26건이 어떤 경로에서는 닫힘, 어떤 경로에서는 열림이었다. 두 번째로 같은 조건을 쓰는 순간이 함수로 뺄 시점이다.
- **교훈**: 내가 예외를 손으로 덧붙이고 있다면 규칙이 잘못된 자리에 있다는 신호다. `|| status === 'CLOSED'`를 두 번 따로 붙였는데, 그게 규칙을 모아야 한다는 증거였다.
- **교훈**: **순수 판정 함수를 DB 계층에 두지 않는다.** dbService를 목으로 바꾼 테스트가 판정 함수까지 잃어 4개가 깨졌다. 데이터베이스가 필요 없는 규칙은 별도 모듈에 두면 목과 무관하게 항상 진짜가 쓰인다.
- **교훈**: 과거 데이터를 현재 포맷으로 바꾸는 것이 언제나 정답은 아니다. 기록되지 않은 종료 시각을 채우는 것은 정리가 아니라 **없는 사실을 만드는 것**이다. 코드가 두 표기를 모두 이해하면 이력은 이력대로 둔다.
