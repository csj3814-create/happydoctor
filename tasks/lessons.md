...
## 2026-09-04 인식되지 않는 필터 값은 조용히 전체를 반환한다
- **교훈**: `applyConsultationViewOptions()`는 `'active'`만 미답변 필터로 인식하는데 새 코드가 `'pending'`을 넘겼다. 어느 분기에도 걸리지 않아 **필터링이 통째로 건너뛰어지고 전체 상담이 반환**됐다. 그 결과 미답변 0건인 상황을 68건으로 3일간 보고했고, 사용자에게 존재하지 않는 백로그를 반복해서 압박했다.
- **교훈**: 분기 목록에 없는 값이 조용히 "전체 통과"가 되는 코드는 위험하다. 열거형 필터는 **알 수 없는 값을 만나면 최소한 경고를 남기도록** 한다.
- **교훈**: 내부 stage 이름(`pending`)과 외부 API 상태값(`active`)이 다르면 반드시 충돌한다. 새 호출부를 만들 때 기존 호출부가 실제로 넘기는 값을 확인한다.
- **교훈**: 사용자가 "다 처리했는데 왜 숫자가 그대로냐"고 하면 사용자를 의심하기 전에 **내 집계 코드를 먼저 의심한다.** 이번엔 사용자가 맞고 내 숫자가 틀렸다.

## 2026-09-14 저장까지만 검증하고 표시를 검증하지 않기
- **교훈**: 기능이 "동작한다"의 기준을 **사용자가 보는 화면까지**로 잡는다. 보듬이 답변 초안은 생성·저장·환자 경로 격리가 전부 테스트로 고정돼 있었지만, 의료진 포털에 표시하는 코드를 아예 만들지 않았다. API가 상담 문서를 통째로 반환한다는 이유로 "별도 직렬화 변경 불필요"라고 판단했는데, 그건 데이터가 응답에 실린다는 뜻일 뿐 화면이 읽는다는 뜻이 아니다. 백엔드 테스트가 모두 통과하는 상태로 아무도 볼 수 없는 기능을 2주간 완료로 보고했다.
- **교훈**: "라이브 검증"을 계획했으면 **검증할 트래픽이 실제로 있는지부터 확인한다.** 9/5 이후 신규 웹 상담이 0건이라 운영 데이터로는 영원히 검증되지 않는 상태였다. 큐가 조용한 것과 기능이 동작하는 것을 구분해야 한다.
- **교훈**: 프로덕션에서 검증하기 전에 **부작용 없는 경로를 먼저 찾는다.** 실제 상담을 접수하면 의료진 알림과 환자 문자가 함께 나간다. 서비스 함수를 합성 데이터로 직접 호출하면 모델·파싱·타임아웃까지 같은 코드로 확인하면서 DB도 알림도 건드리지 않는다.
- **교훈**: 공개 법적 문서는 **인프라를 바꿀 때 같이 바꾼다.** SMTP에서 Resend로 전환한 9/4 이후, 개인정보처리방침은 열흘간 존재하지 않는 위탁 관계(Gmail SMTP)를 고지하고 있었다. 수탁자 목록은 코드가 아니라 계약의 문제라 테스트가 잡아주지 않는다. 외부 서비스 교체 체크리스트에 방침 갱신을 넣는다.
- **교훈**: 규제 선언과 개인정보처리방침이 어긋나면 **어느 쪽이 사실인지 코드로 판정한다.** 이메일 주소 수집은 방침이 맞고 Play 선언이 틀렸다. 문서끼리만 비교하면 어느 쪽에 맞출지 알 수 없다.
- **교훈**: 로그인이 필요한 화면은 **컴포넌트를 분리해 목업 데이터로 띄워 확인한다.** 포털은 Google 로그인이 있어 실제 화면을 열 수 없었지만, 섹션을 별도 파일로 빼고 임시 프리뷰 라우트에서 렌더하니 삽입·복사·실패 상태를 전부 눈으로 확인할 수 있었다. 확인이 끝나면 프리뷰 라우트는 지운다.

## 2026-09-14 화면 밖에 있으면 없는 것이다
- **교훈**: 새 UI를 넣을 때 **첫 화면에 들어오는지**를 배치 기준으로 삼는다. 보듬이 카드를 답변창 옆에 두려고 페이지 맨 아래에 넣었더니, 사용자가 스크롤하다 말고 "기능이 없다"고 판단했다. 렌더링되는 것과 발견되는 것은 다르다.
- **교훈**: 인접성과 발견성이 충돌하면 **발견성을 택한다.** 초안을 답변란에 넣는 버튼은 포커스 이동으로 거리를 없앨 수 있지만, 존재를 모르는 카드는 어떤 방법으로도 구제되지 않는다.
- **교훈**: 비동기로 채워지는 필드는 **비어 있는 동안의 화면을 반드시 설계한다.** 요약은 접수 6초 뒤 저장되는데 의료진 알림은 즉시 나간다. 그 6초 사이에 연 사람에게는 기능이 없는 것처럼 보이고, 화면은 스스로 갱신하지도 않았다. "아직 오는 중"과 "오지 않음"을 구분해 보여줘야 한다.
- **교훈**: 사용자가 "기능이 없다"고 하면 **데이터부터 확인하고 화면을 의심한다.** 이번엔 Firestore에 요약이 멀쩡히 있었고 배포 번들에도 코드가 있었다. 생성 실패로 단정하고 백엔드를 팠다면 엉뚱한 곳을 고쳤을 것이다.
