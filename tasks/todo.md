...
## 지금 열린 항목 (2026-09-14 기준)

아래 기록은 시간순으로 계속 쌓인다. 지금 남은 일만 보려면 이 절만 읽으면 된다.

### 제출 차단 — 사람이 판단해야 하는 것

- [ ] 의료법 검토 (보듬이 승인제 구조 기준) — 변호사 검토, 제출 차단 1순위
- [ ] `자동 임상 의사결정 지원: 아니요` 답변을 규제 검토로 확정한다
- [ ] Gemini API 키 요금 등급 확인 — 무료 등급이면 입력이 제품 개선에 쓰일 수 있어 수탁 처리 전제가 무너진다
- [ ] Play Console에 Data Safety 양식을 직접 입력한다 (`docs/google-play/data-safety-ko.md` 기준, 이메일 주소 항목 포함)
- [ ] 의료진 자격 확인·승인·회수·비밀유지 문서를 확정한다
- [ ] 조직 개발자 계정 / D-U-N-S (PWA 유지 방침이라 비긴급)

### 운영 — 남은 확인과 선택 사항

- [ ] 카카오 상담에 포털에서 답변을 보내 **문자 알림이 실제로 나가는지** 확인한다 (연락처 저장까지는 확인됨)
- [ ] `_dmarc` TXT 레코드 추가 (선택, `v=DMARC1; p=none;`)
- [ ] 카카오페이·토스 송금 링크를 후원 안내에 추가한다 (링크 확보 필요)
- [ ] 국제번호 환자 도달 수단 — SOLAPI가 국내 전용이라 이메일 외에는 방법이 없다

### 홍보·제휴 — 2026-05 이후 멈춰 있음

- [ ] 1차 발송 4곳 후속 연락 (서울외국인주민센터·서울글로벌센터·영등포구 가족센터·요셉의원, 예정일 2026-05-26 경과)
- [ ] 2차 발송군 4곳 (동부외국인주민센터·성북구 가족센터·라파엘클리닉·시립서대문농아인복지관) — 본문은 `docs/happydoctor_partner_priority8_sendpack_2026-04-17.md`에 완성
- [ ] 보도자료 배포 여부 결정 (`docs/happydoctor_press_release_deaf_committee_final.md`, 기자 메일 초안 준비됨)
- [ ] `docs/`의 홍보·제휴 문서와 `scripts/generate_press_release_docx.py`가 아직 git에 없다

> **2026-09 이전 절의 체크박스는 신뢰하지 말 것.** 단계 47~70 등 옛 절에는 미체크 항목이 100개 넘게 남아 있지만, 각 절의 Review에는 완료로 기록돼 있다. 계획 체크박스만 누르지 않은 것이라 실제 상태는 Review를 봐야 한다. 추정으로 일괄 체크하지 않고 이 경고만 남긴다.

---

## 2026-07-28 Google Play 출시 차단 요소 수정

- [x] 구 Cafe24 관리자 비밀번호를 변경한다. (사용자 계정 작업)
- [x] 평문 자격증명 파일을 제거하고 공개 Git 이력 정리 절차를 완료한다.
- [x] AI 분석 실패를 보수적 의료진 검토·응급 안내로 전환하고 테스트한다.
- [x] 상담 조회 코드를 장기 토큰 우선·만료·전용 제한 구조로 강화한다.
- [x] 환자 앱과 백엔드 취약 의존성을 안전한 버전으로 업데이트하고 전체 검증을 통과한다.
- [x] 개인정보처리방침·데이터 삭제 안내·삭제요청 기능을 공개 URL로 제공한다.
- [x] Data Safety·건강 앱 선언·스토어 등록 문안을 저장소에 작성한다.
- [x] 새 Android 패키지명 `kr.happydoctor.app`을 출시 후보로 반영한다. (실제 생성 가능 여부는 Play Console에서 최종 확인)
- [ ] API 36 기반 TWA 프로젝트·서명·assetlinks·AAB 생성 경로를 구축한다.
- [x] Play 규격 아이콘·기능 그래픽·실제 앱 스크린샷을 준비한다.
- [ ] 조직 인증 완료 후 내부 테스트 트랙에 제출한다. (D-U-N-S/Play Console 외부 상태 의존)

### Review
- 공개 `main` 이력을 재작성해 `scripts/crawl_qna.js` 경로가 0건임을 확인했고, 최종 트리 해시가 보안 수정 커밋과 동일한 상태로 강제 갱신했다. GitHub의 강제 푸시 허용은 작업 중에만 켰다가 즉시 다시 비활성화했고 필수 상태 검사 4개도 유지했다.
- AI 임상 응답 경로를 제거하고 모든 신규·후속 상담을 의료진 직접 검토로 전환했다. 장기 비밀 토큰, 6자리 코드 24시간 만료, 실패 조회 rate limit, URL fragment·전용 헤더 경로를 적용했다.
- 백엔드 57개 테스트·구문·모듈 로드, 환자 앱 린트·타입·프로덕션 빌드가 통과했다. 프런트와 백엔드의 `npm audit --omit=dev`는 모두 취약점 0건이다.
- Windows에서 생성한 잠금파일의 선택 의존성 누락을 보완했고, 커밋 `040daa2`에 대한 GitHub Frontend CI의 홈페이지·환자 앱·포털 작업이 모두 통과했다.
- 개인정보처리방침 `/privacy`, 삭제요청 `/delete-data`, 실제 삭제 API와 영수증 토큰 확인 흐름을 구현했다. 국외 처리 리전·의료진의 제3자 지위·법정 진료기록 보존기간은 제출 전 확정해야 한다.
- `kr.happydoctor.app`, compile/target SDK 36, min SDK 26 TWA가 빌드됐고 unsigned AAB 해시를 검증했다. 비밀번호를 명령행이나 Git에 남기지 않는 대화형 업로드 키 생성·서명 스크립트를 준비했다.
- Play 기능 그래픽 1024×500, 실제 앱 모바일 스크린샷 1080×1920 4장, Data Safety·건강 앱 선언·한국어 스토어 설명 초안을 준비했다.
- 남은 외부 작업은 구 Cafe24 관리자 비밀번호 변경, D-U-N-S/조직 계정 인증, 업로드 키의 안전한 생성·백업, Play 앱 서명 SHA-256을 반영한 assetlinks 배포, 법률·국외처리 정보 확정과 내부 테스트 업로드다.
- Render의 `FIREBASE_SERVICE_ACCOUNT`를 안전하게 복원했고 Firebase Admin 14 모듈 API 호환성과 신뢰 프록시 설정을 수정했다. 운영 `/api/version`은 리비전 `79197b2`와 `dbConfigured:true`를 반환하고, `/api/stats`가 Firestore 누적값 `419/330`을 반환해 실제 읽기 연결도 확인했다.
- 구 Cafe24 관리자 `adm_medi`의 노출 비밀번호를 새 고유 비밀번호로 변경하고, 로그아웃 후 새 비밀번호 재로그인까지 확인했다. 사용하지 않는 다른 관리자 계정은 사용자 결정에 따라 현 상태로 유지한다.
- 업로드 키 `android/twa/android.keystore`가 생성됐고 Git 제외 규칙 적용을 확인했다. 사용자가 키 파일과 비밀번호의 별도 백업 완료를 확인했으며, 비밀번호와 키 내용은 작업 기록에 남기지 않았다.
- 서명된 번들 `android/twa/app/build/outputs/bundle/release/app-release-signed.aab`을 생성했다. `jarsigner` 검증 종료코드 0, Gradle `bundleRelease` 성공, API 36 설정 검증을 통과했으며 AAB SHA-256은 `229630C08C01FD3EF991F16F8694DBF970E948E51B01CB73665D6EB62938A0FD`다.
- 현재 업로드 인증서 SHA-256은 `D2:74:64:B5:4A:AA:F3:24:96:A2:7C:C1:52:99:B8:88:09:60:06:6E:E9:B4:63:7B:81:4C:D6:24:AF:76:61:E2`다. 공개 `assetlinks.json`에는 내부 테스트 업로드 후 Play Console에서 확인한 **Play 앱 서명 인증서** 지문을 사용해야 한다.

## 2026-07-28 Google Play 출시 준비 감사

- [x] Android 앱 프로젝트와 중첩 작업 지침을 식별한다.
- [x] `applicationId`, 버전, SDK, Gradle·Kotlin 및 주요 의존성을 점검한다.
- [x] 광고 SDK 포함 여부와 제거 또는 업그레이드 필요성을 확인한다.
- [x] 디버그·릴리스 빌드와 테스트를 실행해 재현 가능한 결과를 남긴다.
- [x] 서명·AAB 생성 준비 상태를 점검하되 비밀키 내용은 열람하지 않는다.
- [x] 개인정보처리방침, 데이터 보안, 건강 앱 신고 및 스토어 등록자료의 준비 상태를 점검한다.
- [x] 출시 차단 요소와 권장 작업 순서를 정리한다.

### Review
- 직접 만든 환자용 앱은 `frontend/app`의 Next.js PWA이며 Android/Gradle/TWA 래퍼, `applicationId`, Android 버전·SDK, 서명 설정과 AAB는 아직 없다.
- 현재 PWA와 저장소에는 Google Mobile Ads SDK가 없으므로 삭제된 레거시 앱의 `play-services-ads:19.4.0` 경고와 무관하다.
- `frontend/app npm run verify:ci`가 통과했고, 백엔드는 `npm run verify:ci`에서 50개 테스트·구문·모듈 로드를 모두 통과했다.
- 2026-07-28 `npm audit --omit=dev` 기준 환자 앱은 high 3건, 백엔드는 critical 2건·high 9건을 포함한 26건이 검출돼 의존성 업데이트와 회귀 검증이 필요하다.
- TWA 필수 파일 `https://app.happydoctor.kr/.well-known/assetlinks.json`과 개인정보처리방침 공개 URL은 모두 404다.
- PWA manifest가 아이콘을 512x512, 스크린샷을 1179x2556으로 선언하지만 실제 파일은 각각 1024x1024, 1200x630이다.
- 개인정보처리방침·이용약관·삭제요청 경로·Data Safety 문안·건강 앱 선언·Play 전용 설명/512 아이콘/1024x500 기능 그래픽/실화면 스크린샷이 준비되지 않았다.
- 건강정보·사진·선택적 전화번호가 Firebase, Gemini, Google Cloud Translation, SOLAPI, Vercel Analytics 흐름에 포함되므로 수집 목적·외부 처리자·보유기간·삭제 절차를 정확히 공개해야 한다.
- 임상 안전 차단 요소로 Gemini JSON 파싱 실패 시 `AUTONOMOUS_REPLY`로 응급 상황이 아닌 듯하다고 안내하는 fallback이 확인됐다. 실패 시 보수적 에스컬레이션으로 바꿔야 한다.
- 보안 최우선 차단 요소: 공개 GitHub 저장소의 추적 파일 `scripts/crawl_qna.js`에 구 `happydoctors.net` Q&A 관리자 ID/비밀번호가 평문으로 포함돼 있고 Git 이력에도 남아 있다. Cafe24 호스팅은 2026-10-15까지 살아 있으므로 자격증명은 유효한 것으로 보고 변경한 뒤, 코드·Git 이력에서도 제거해야 한다.
- 6자리 무기한 추적코드로 건강정보·의료진 답변·사진 접근이 가능해 긴 토큰 우선, 코드 만료, 전용 rate limit 또는 추가 인증을 설계해야 한다.
- 권장 순서: 노출 자격증명 회전 → 위험한 AI fallback·접근통제 개선 → 의존성 업데이트 → 개인정보처리방침/삭제정책 작성 → 조직 인증·새 패키지명 확정 → TWA/서명/assetlinks/AAB 생성 → Data Safety·건강 앱 선언·스토어 자료 준비 → 내부 테스트.

## 2026-07-28 대표메일 운영 상태 확인

- [x] `president@happydoctor.kr`의 현재 포워딩 구성을 확인한다.
- [x] 연결된 Gmail에서 실제 수신 흔적을 확인한다.
- [x] 수신과 발신의 운영 방식을 사용자에게 정리한다.

### Review
- 현재 DNS는 `mx1.forwardemail.net`, `mx2.forwardemail.net`을 사용하며 `president@happydoctor.kr`를 `csj3814@gmail.com`으로 전달하도록 설정돼 있다.
- 연결된 Gmail에서 2026-05-19 테스트 메일과 6~7월 실제 수신 메일이 확인돼 포워딩이 동작 중이다.
- 독립 메일함은 아니며 수신은 Gmail에서 직접 관리한다. 발신은 별도 `Send mail as` 설정이 없으면 `csj3814@gmail.com` 주소로 나간다.

## 2026-07-28 NICE D&B 공식 문의 등록

- [x] 사용자가 확정한 문안으로 Q&A 내용을 교체한다.
- [x] 사용자가 직접 NICE D&B Q&A를 등록했다.
- [x] 중복 등록을 방지하기 위해 에이전트의 추가 전송을 중단한다.

### Review
- 확정 문안을 입력한 뒤 에이전트가 `등록`을 한 차례 클릭했으나 완료 메시지나 화면 전환이 없고 입력 폼이 그대로 유지됐다.
- 사용자가 직접 등록 완료 사실을 알려 중복 전송 없이 작업을 종료했다.

## 2026-07-28 Google Play 조직 인증 준비 실행

- [x] 홈페이지의 법적 지위 표현을 실제 상태에 맞게 정정한다.
- [x] 대표 전화와 대표메일을 홈페이지 하단에 공개한다.
- [x] 수정된 홈페이지를 빌드·화면 검증한다.
- [x] 변경 파일만 커밋·푸시하고 공개 홈페이지 반영을 확인한다.
- [ ] D&B에서 기존 D-U-N-S 번호를 조회하고, 없으면 신청서를 제출 직전까지 준비한다. (한국 공식 파트너 NICE D&B 문의 등록 승인 대기)
- [ ] Google Play 조직 인증에 사용할 정보와 증빙 목록을 확정한다.

### Review
- 홈페이지 하단의 `비영리법인 고유번호`를 `비영리단체 고유번호`로 정정하고 대표 전화 및 대표메일을 추가했다.
- 연혁의 `비영리법인으로 전환` 표현도 실제 결과와 맞도록 `비영리법인 전환을 추진`으로 정정했다.
- `frontend/homepage`에서 `npm run verify:ci`가 통과했고, 로컬 `/ko` 화면에서 네 항목의 실제 노출을 확인했다.
- 홈페이지 변경만 커밋 `4e87500`으로 `main`에 푸시했으며, 공개 `https://happydoctor.kr/ko`에서 수정 문구와 연락처가 반영된 것을 확인했다.
- D&B의 Google 개발자용 공식 국가 선택에서 대한민국은 NICE D&B로 연결된다. 국내 번호 조회·발급은 회원가입이 필요해, 고유번호증 보유 비법인 비영리단체의 발급 가능 여부·기존 번호 조회·Google 개발자 목적 무료 여부·필요 서류를 묻는 Q&A를 등록 직전까지 작성했다.

## 2026-07-28 고유번호증 기반 조직 계정 인증 재검토
- [x] 홈페이지 하단의 단체명·주소·전화번호 확인
- [x] 고유번호증 기반 D-U-N-S 신청 준비정보 대조
- [x] Google Play 조직 계정 인증 실행 순서 확정

### Verification
- [x] 저장소와 라이브 홈페이지의 공개 정보 일치 확인
- [x] Google/D&B 공식 요구정보와 현재 보유정보 대조

### Review
- 저장소와 라이브 `https://happydoctor.kr/ko` 하단에서 고유번호 `111-82-67141`, 대표자 최석재, 서울 영등포구 공식 주소가 동일하게 확인됐다.
- 공식 전화번호는 운영 문서에는 있으나 현재 홈페이지 하단에는 표시되지 않는다. 조직 인증용 공개 연락처를 홈페이지 또는 별도 연락처 페이지에 추가해야 한다.
- 홈페이지의 `비영리법인 고유번호` 표현은 사단법인 설립이 완료되지 않은 실제 상태와 다르므로 `비영리단체 고유번호` 또는 고유번호증의 정확한 법적 표기로 정정하는 것이 안전하다.
- Google은 비영리단체도 조직 계정 대상으로 안내하며, D&B의 D-U-N-S 무료 신청은 조직 규모나 매출이 아니라 확인 가능한 법적 이름·주소·전화·대표자·법적 형태를 요구한다.
- 고유번호증, 공식 주소·전화, 대표자, 홈페이지, 단체 계좌와 활동 이력이 있으므로 D-U-N-S를 신청할 실질적인 기반은 갖춰져 있다. 발급 확정 여부는 D&B의 문서 검증 결과에 따른다.
- 신청 시 단체명과 주소는 고유번호증의 표기를 그대로 사용하고, D&B 정보·Google 결제 프로필·Play Console 조직 정보가 문자 단위로 일치하도록 맞춘다.
- 실행 순서: 고유번호증의 정확한 단체명·주소 확인 → 홈페이지 법적 표기와 전화번호 정비 → D&B 무료 D-U-N-S 신청 → 번호 발급·정보 반영 확인 → Play Console에서 조직 계정 선택·인증.

## 2026-07-28 건강 앱 개인 개발자 계정 가능 여부 확인
- [x] Google Play 건강·의료 앱의 계정 유형 요구사항 확인
- [x] D-U-N-S 없는 소규모 비영리단체의 예외 인증 경로 확인
- [x] Happy Doctor에 적합한 계정 유형과 위험 정리

### Verification
- [x] Google 공식 계정 유형·건강 앱·개발자 인증 문서 대조
- [x] Play Console의 현재 선택 화면과 공식 문구 구분

### Review
- 현재 계정 인증 화면은 개인 계정을 선택할 수 있게 되어 있고 기존 계정 유형 안내도 건강 앱에 조직 계정을 `선택해야 한다(should)`고 표현한다.
- 그러나 2026-09-30 시행 예정인 Play Console Requirements는 건강·의료 서비스를 제공하는 개발자가 조직 계정으로 등록해야 한다고 `must`로 명시한다.
- Happy Doctor는 증상 입력과 의료진 상담을 제공하므로 건강·의료 앱에 해당하며, 카테고리만 바꾸어 개인 계정으로 우회하는 것은 적절하지 않다.
- 개인 계정 선택은 앱을 개인이 실제로 소유·운영한다는 전제에서 2026-09-30 전 내부 테스트용 임시 다리로는 검토할 수 있으나, 지속적인 Play 배포 해법은 아니다.
- 조직 계정의 D-U-N-S는 조직 규모·매출과 무관하며 무료 신청이 가능하지만, D&B가 확인할 수 있는 법적 조직명·주소·전화·법적 형태가 필요하다. Google은 최대 30일을 예상하도록 안내한다.
- D-U-N-S 대체 인증은 D&B가 지원하지 않는 지역의 조직 또는 알려진 정부기관에 한정된다. 한국의 소규모 비영리라는 이유만으로 일반 면제가 제공되지는 않는다.
- 현실적인 경로는 `법적 조직 정비 후 D-U-N-S 신청`, `실제 운영·소유권을 맡을 협력기관의 조직 계정으로 정식 배포`, `준비될 때까지 PWA 유지` 세 가지다.

## 2026-07-28 새 Happy Doctor 앱 Play Console 등록 준비
- [x] Google Play 개발자 등록비 정책 확인
- [x] 복구된 콘솔의 실제 계정·결제·앱 생성 가능 상태 확인
- [x] 현재 저장소의 Android 빌드 경로와 제출 산출물 확인
- [x] 새 앱 등록 및 내부 테스트 제출 순서 확정

### Verification
- [x] 콘솔에서 재결제 요구 여부와 계정 경고 상태 확인
- [x] Android 패키지명·서명·AAB 생성 가능 여부 확인
- [x] 실제 앱 생성·업로드 전 사용자 확인 지점 명시

### Review
- 계정 ID `7791206817556472817`의 기존 개발자 계정으로 로그인되며, Google Play 등록비는 일회성 USD 25이므로 같은 계정에서 재결제할 필요는 없다.
- 콘솔은 재등록 결제가 아니라 개발자 계정 인증을 요구한다. 2025-01-03까지 인증을 완료하지 않아 계정이 `제한된 개발자 계정` 상태이고, 새 앱 만들기 버튼이 잠겨 있다.
- 기존 두 Happy Doctor 앱은 사라진 것이 아니라 목록에 남아 있으며 둘 다 `Google에서 삭제` 상태다. 사용자 요청대로 기존 앱에는 손대지 않는다.
- 인증 흐름은 `계정 유형 선택 → 결제 프로필 연결 → 계정 정보 → 공개·비공개 연락처` 순서다. 의료·건강 앱은 Google 공식 안내상 조직 계정을 선택하는 것이 적절하며 D-U-N-S 번호가 필요하다.
- 현재 환자용 앱은 `https://app.happydoctor.kr`에서 운영되는 Next.js PWA다. Android/Gradle/Capacitor 원본과 AAB는 없으므로 새 Play 앱을 위해 Android 래퍼를 새로 만들어야 한다.
- 웹앱 매니페스트는 정상 응답하지만 Digital Asset Links(`/.well-known/assetlinks.json`)는 404다. TWA 방식으로 패키징하려면 서명키 생성 후 이 파일을 배포해야 한다.
- 제출 전 개인정보처리방침 공개 URL, 데이터 보안 양식, 건강 앱 선언, 스토어 등록정보, API 36 대상 AAB가 필요하다.
- 권장 순서: 조직 계정 인증 완료 → 새 패키지명 확정 → TWA 프로젝트·서명키 생성 → assetlinks 배포 → AAB 검증 → 새 Play 앱 생성 → 내부 테스트 제출.

## 2026-07-28 Google Play 계정 복구 관련 메일 확인
- [x] Gmail에서 최근 Google Play·개발자 지원 메일 검색
- [x] 관련 스레드 본문과 회신 상태 확인
- [x] 기한·요청사항·복구 가능성에 따른 대응 방향 정리

### Verification
- [x] 검색 범위와 확인한 최신 메일 시각 기록
- [x] 사용자 승인 전 메일 발송·보관·라벨 변경 없음

### Review
- 검색 범위: 2026-07-14 이후 Gmail의 Google Play·개발자 계정·SDK 관련 메일 12건.
- 최신 지원 메일: `Re: [5-4039000040999] Google Play에 관한 메시지`, 2026-07-28 10:13 KST 수신, 미열람 상태.
- Google Play Console 지원팀이 기존 개발자 계정 복구 완료를 확정했고 기존 계정을 계속 사용할 수 있다고 안내했다.
- 계정을 다시 휴면 상태로 만들지 않으려면 앱 초안 생성만으로는 부족하며, 어떤 트랙이든 앱 번들을 업로드해 제출해야 한다고 명시했다.
- 지원팀에 대한 감사 회신은 선택 사항이다. 우선 Play Console 접속, 연락처 인증, 앱 현황 확인, 내부 테스트 트랙 AAB 제출을 진행한다.
- 메일 읽기 외 발송·보관·라벨 변경은 수행하지 않았다.

## 2026-07-26 Google Play Ads SDK 경고 대응 점검
- [x] Google Play 메일의 경고 대상과 출시 영향 구분
- [x] 현재 저장소에서 Android/Gradle 및 `play-services-ads` 의존성 위치 확인
- [x] Google 공식 문서 기준 최신 지원 버전과 업그레이드 요구사항 확인
- [x] Happy Doctor에 맞는 업데이트·테스트·출시 순서 정리

### Verification
- [x] 저장소 전체에서 `play-services-ads`, Android Gradle 플러그인, 패키지명 검색
- [x] Google Mobile Ads SDK 공식 릴리스 노트 및 지원 중단 안내 대조

### Review
- Google Play 경고는 `com.ablestor.happydoctor` versionCode 2와 `com.happydoctor.app` versionCode 8 두 패키지에 각각 적용된다.
- 두 번들 모두 2020년에 출시된 `play-services-ads:19.4.0`을 포함하며, v7~v19 계열은 2023-06-30에 이미 sunset되었다.
- 현재 저장소와 Git 이력, `C:\SJ` 내 Android 산출물에는 두 패키지의 Android 원본 또는 AAB/APK가 없다. 현재 저장소는 웹·백엔드/PWA 코드이므로 여기서 SDK를 직접 교체할 수 없다.
- 추가 확인 결과 기존 개발자 계정은 2024-04-24 비활성 상태로 해지되어 앱별 콘솔 접근, SDK 업데이트, 게시 중단, 일반 앱 이전을 진행할 수 없다.
- 따라서 두 SDK 메일은 해지 계정에 남아 있던 과거 번들을 대상으로 한 자동 알림으로 보고 별도 회신하지 않는다.
- 공식 계정 종료 이의제기 정책상 2026-01-28 이전 종료 건의 일회성 신청 기한이 2026-07-27이므로, 복구가 보장되지는 않더라도 2026-07-26 안에 지원 요청을 먼저 제출한다.
- 복구되지 않으면 새 Google 계정으로 새 Play 개발자 계정을 만들 수 있지만, 기존 계정이 활성 상태가 아니어서 표준 앱 이전은 불가능하다. 새 앱과 새 패키지로 다시 게시하는 경로를 기본안으로 잡는다.
- 새 앱을 만들 때 광고를 사용하지 않는다면 Ads SDK를 넣지 않고, 2026-08-31 정책을 고려해 `compileSdk/targetSdk 36` 기준으로 빌드한다.
...
- [x] Stage 120 대표 소개 외부 활동 표기 최소 정정 (2026-07-19)
  ...
  - Result: 사용자 요청에 따라 커밋 `93c7a64`를 `main`에 푸시해 Vercel 자동 배포를 실행했다.
  - Result: 라이브 `https://happydoctor.kr/ko`가 HTTP 200을 반환하고 `최석재의 골든타임`, `습관학교 해빛스쿨` 새 표기를 포함하며, 옛 단독 링크명 `건방진 닥터스`가 사라진 것을 확인했다.

## 2026-09-01 알림 프로세스 점검 및 이메일 알림 채널 추가

### 점검 결과 (프로덕션 Firestore 직접 조회)
- 보듬이 AI 트리아지는 고장이 아니라 `d78e4e9`에서 의도적으로 제거됨. `llmService.js`는 LLM 호출 없이 고정 템플릿만 반환한다.
- `delivery_rooms/doctor_room` 미등록. `resolveDoctorAlertDeliveryRoom()`이 개인 알림방을 우선하므로 모든 의료진 알림이 `messenger_rooms/operator_alerts`(`가족-최석재`) 개인방으로만 배달됐다.
- 의료진 알림 리마인더는 0/5/15분, 후속 푸시는 15분/3시간/24시간이 전부다. 그 창을 놓치면 미답변 상담은 영구히 무알림 상태가 된다.
- `patient_sms_notifications` pending 24건, attempts=0. Render에 SOLAPI 환경변수가 없어 SMS 루프가 비활성 상태다. 2026-09-01 답변 2건도 환자에게 미전달.
- 전체 발송이 MessengerBot 안드로이드 폰 폴링에 의존하는 단일 장애점 구조다.
- 포털 미답변 67건(ACTIVE 58) 확인.

### 작업 항목
- [x] `brace-expansion` 5.0.9 override로 프로덕션 high 취약점 1건 제거
- [x] `config.js`에 SMTP·알림 수신자·일일 요약 설정을 추가한다
- [x] `services/emailService.js`를 만들어 Gmail SMTP 발송 경로를 구현한다 (미설정 시 무해하게 비활성)
- [x] 의료진 알림 큐 적재 시 이메일을 동시 발송한다 (카톡 봇과 독립)
- [x] `services/unansweredDigestService.js`로 미답변 일일 요약 메일을 발송한다 (중복 발송 방지 가드 포함)
- [x] 환자 답변 알림에 이메일 대체 경로를 추가한다 (카톡 → SMS → 이메일)
- [x] 환자 상담 시작 폼에 답변 알림용 이메일 입력을 추가한다
- [x] 모든 알림 메일은 건강정보를 담지 않고 포털 링크만 안내한다
- [x] 테스트·구문·모듈 로드 검증을 통과시킨다
- [~] Render에 `SMTP_USER`/`SMTP_PASS`를 설정한다 → **무효**. Render가 SMTP를 막아 Resend HTTPS로 전환했다 (2026-09-04)
- [ ] 의료진 단톡방에서 `~알림방등록`을 보내 단톡방 알림을 복구한다 (사용자 직접 작업)
- [x] Render에 SOLAPI 3종 변수를 설정해 환자 SMS 24건을 살린다 (설정 완료, 발신번호 등록 후 발송 성공)

### Review
- 프로덕션 Firestore 직접 조회로 원인을 확정했다. 알림은 "발송 실패"가 아니라 `delivery_rooms/doctor_room` 미등록 때문에 개인 카톡방 `가족-최석재` 한 곳으로만 배달되고 있었다. 큐에는 전부 `delivered`로 남아 실패 흔적이 없었다.
- 카카오 봇과 독립된 이메일 경로를 추가했다. `enqueueDoctorNotification()` 한 곳에 훅을 걸어 6개 호출 지점을 건드리지 않고 전 경로에 적용했고, 메일 실패가 카톡 큐를 되돌리거나 막지 않는 것을 테스트로 고정했다.
- 일일 요약은 Firestore `system_jobs/unanswered_digest` 문서를 트랜잭션으로 claim해 Render 재시작 시 중복 발송을 막는다. KST 날짜 경계 처리도 테스트로 고정했다.
- 알림 메일에는 건강정보를 넣지 않는다. 기존 카카오 운영 알림 원칙과 동일하며, "건강정보와 상담 내용은 이 메일에 담기지 않습니다" 문구와 포털 링크 존재를 테스트로 검증한다.
- 환자 답변 알림은 카카오 채널 → SMS → 이메일 3단 폴백이 됐다. 상담 시작 폼에 이메일 입력을 추가하고 휴대폰·이메일 중 하나만 입력해도 되도록 검증을 완화했다.
- 개인정보처리방침에 이메일 수집 항목과 Gmail SMTP 수탁자를 반영했다.
- 백엔드 78개 테스트·구문·모듈 로드 통과, `npm audit --omit=dev` 0건. 환자 앱 린트·타입체크·프로덕션 빌드 통과.
- 남은 외부 작업은 Gmail 앱 비밀번호 발급과 Render 환경변수 설정, 의료진 단톡방 `~알림방등록`, SOLAPI 변수 설정이다. 상세 절차는 `docs/happydoctor_notification_channels_2026-09-01.md`에 있다.
- 알림 문구 번역은 한국어·영어만 채웠다. 나머지 16개 언어 번들은 영어 문구로 대체 표시되며, 번역 확보 후 `lib/start-copy-localized.ts`에 채워야 한다.

## 2026-09-01 알림 실패의 가시화 (재발 방지 1·2단계)

- [x] 선택 채널의 반쪽 설정이 서버 기동을 중단시키지 않도록 바꾼다 (SMTP·SOLAPI 모두)
- [x] 설정 누락 사유를 `/api/version`의 `notifications` 블록으로 노출한다
- [x] 큐 적체·미답변 경과·방 등록 여부를 `/api/notification-health`로 노출한다 (API 키 보호)
- [x] 운영 문서에서 단톡방 복구 안내를 철회하고 확인 절차로 대체한다
- [x] MessengerBot 폰의 `lastPolledAt`을 기록하고 폴링 중단 시 메일 경고를 보낸다 (2026-09-02 구현)

### Review
- 반쪽 설정으로 서버가 죽는 위험을 제거했다. `getSmtpConfig()`·`getSolapiSmsConfig()`는 더 이상 던지지 않고 `null`과 사유를 반환하며, `validateStartupConfig()`는 사유를 `console.error`로 남기되 기동을 계속한다. Render 대시보드에서 손으로 입력하다 한쪽만 저장하는 상황이 실제로 임박해 있었고, 그 대가가 상담 접수 전체 중단이었다.
- SOLAPI도 함께 고쳤다. 같은 위험이 사용자의 다음 작업(SMS 변수 설정)에 그대로 남아 있었기 때문이다.
- `/api/version`은 비밀값 없이 동기로 채널 상태만 반환한다. 환경변수를 저장한 직후 브라우저로 열어 `ready` 여부와 실패 사유를 바로 확인할 수 있다.
- `/api/notification-health`는 Firestore를 읽고 운영 상태를 드러내므로 `MESSENGER_API_KEY`로 보호했다. 방 이름은 개인을 식별하므로 불린으로만 노출하고, 페이로드에 이름이 섞이지 않는 것을 테스트로 고정했다.
- 로컬 스모크로 실제 HTTP 응답을 확인했다. 반쪽 설정 시 `validateStartupConfig()`가 `true`를 반환하고 `/api/version`이 `configured:false` + 정확한 사유를 냈으며, 양쪽을 채우면 `ready:true`로 바뀐다.
- 백엔드 89개 테스트·구문·모듈 로드 통과.
- 카카오 목적지 관련 이전 안내를 철회했다. `가족-최석재`는 `ea01485`의 의도적 결정이며, 개인 알림방 해제 명령 자체가 존재하지 않는다.

## 2026-09-02 환자 답변 SMS 큐 차단 해소

### 오진 정정
- 2026-09-01에 "SOLAPI 환경변수 미설정으로 SMS 워커가 비활성"이라고 진단했으나 **틀렸다.** 배포 후 `/api/version`이 `sms.configured: true`를 반환해 드러났다.
- 원인은 진단 스크립트가 존재하지 않는 필드 `deliveryAttempts`를 읽어 모든 문서가 `attempts=0`으로 보인 것이다. 실제 필드명은 `attemptCount`이고, 가장 오래된 건은 `attemptCount=854`였다.

### 진짜 원인
- `acknowledgePatientSmsNotification()`이 발송 실패 시 재시도 상한도 백오프도 없이 `status`만 `pending`으로 되돌렸다. `availableAt`이 그대로라 실패한 건이 due 큐 맨 앞에 영구히 고정됐다.
- `executeClaimedNotification()`이 예외를 다시 던져 배치 전체가 중단됐고, 뒤의 모든 메시지가 시도조차 되지 못했다.
- `initialize()`에서 그 예외가 빠져나가 `startProcessorLoop()`이 아예 호출되지 않았다. 재시작해도 같은 건에서 다시 실패해 루프가 영구히 죽었다.
- 결과: 2026-06-04 이후 환자 답변 SMS **delivered 0건**. 채널이 한 번도 동작한 적이 없다.

### 작업 항목
- [x] 재시도 5회 상한과 지수 백오프(1/2/4/8/16분)를 적용하고, 소진 시 `failed`로 죽은편지함 처리한다
- [x] 실패 시 배치를 중단하지 않도록 `executeClaimedNotification()`에서 예외를 삼킨다
- [x] 초기 드레인이 실패해도 `startProcessorLoop()`이 실행되도록 한다
- [x] 포기한 건을 메일로 보고한다 (전화번호·답변 본문 미포함)
- [x] `failed` 건수를 `/api/notification-health`에 노출한다
- [x] 밀린 백로그를 정리한다 (7일 초과 21건 + 중복 리마인더 단계 8건 취소, 환자 4명에게 1건씩만 유지)

### Review
- 사용자 결정에 따라 6~7월자 21건(환자 7명)은 취소했다. 3개월 지난 "답변 도착" 문자는 환자에게 혼란을 주고 그사이 증상이 변했을 수 있다.
- 각 답변은 0/5/15분 리마인더 3건으로 큐잉되는데 모두 만료 상태라 그대로 두면 환자 한 명이 동일 문자를 3통 연속 받게 된다. 1단계만 남기고 8건을 추가 취소해 환자 4명에게 각 1통씩만 가도록 했다.
- 정리 결과 `{cancelled: 35, pending: 4}`. 854회 실패하던 차단 원인 문서도 이 정리로 제거됐다.
- 백엔드 96개 테스트 통과. 포이즌 필 시나리오(실패 건 뒤의 메시지가 정상 발송되는지), 죽은편지함 전환, 백오프로 `availableAt`이 앞으로 이동하는지, 초기 드레인 실패 후에도 루프가 뜨는지를 각각 테스트로 고정했다.
- 기존 테스트 `processDueNotifications requeues the SMS job when SOLAPI send fails`는 예외 전파를 검증하고 있었다. 그 동작이 바로 장애 원인이므로 테스트를 새 동작에 맞게 바꿨다.

## 2026-09-02 SMS 발송 실패 원인 규명 및 도달 불가 수신자 처리

- [x] SOLAPI `failedMessageList`의 `statusCode`/`statusMessage`를 실패 사유로 기록한다
- [x] 실패 사유를 300자로 제한한다 (SDK 검증 오류가 56KB로 저장되던 문제)
- [x] 국제번호는 큐잉하지 않고, 이미 큐에 있으면 즉시 죽은편지함 처리한다
- [x] 실패 사유를 실제로 기록해 원인을 규명한다 → **`1062 발신번호 미등록`으로 확정**
- [x] `/api/notification-health`의 `oldestUnansweredAgeMinutes`가 항상 null이던 버그를 고친다
- [x] SOLAPI에 발신번호 `01025899695`를 사전등록한다 (2026-09-04 등록, 최초 delivered 확인)

### Review
- 큐 차단이 풀리자 4건 모두 시도됐고 두 종류의 실패가 드러났다. 국제번호 2건은 SDK 검증 단계에서 거부되고, 국내번호 2건은 SOLAPI가 접수 후 거부한다.
- 국제번호는 SOLAPI가 한국 국내 서비스이므로 재시도가 결과를 바꿀 수 없다. `isSmsDeliverableNumber()`로 큐잉 단계에서 걸러 포털의 알림 폴백이 곧바로 이메일로 넘어가게 했다. 이미 큐에 있는 건은 `exhaust` 플래그로 즉시 죽은편지함 처리한다.
- 실패 사유 저장이 무제한이라 SDK 타입 검증 오류 하나가 Firestore 문서에 56KB로 들어가 있었다. 진단 스크립트 출력이 폭발한 원인이기도 하다. 300자로 제한했다.
- 국내번호 실패의 정확한 사유는 `failedMessageList`에 있으나 저장하지 않아 알 수 없었다. 이제 `statusCode statusMessage` 형태로 기록되며 수신자 번호는 제외한다. 다음 실패부터 SOLAPI 콘솔 접근 없이도 원인을 특정할 수 있다.
- 백엔드 101개 테스트 통과.
- 남은 문제: 국제번호 환자에게는 도달 수단이 없다. 상담 폼의 이메일 입력은 신규 상담에만 적용되므로, 기존 환자는 포털에서 직접 연락해야 한다.

## 2026-09-02 SMS 발송 실패 원인 확정

- 국내번호 2건을 재큐잉해 새 실패 기록 코드를 태운 결과 **`1062 발신번호 미등록`**이 나왔다. SOLAPI 계정에 로그인하지 않고 원인을 특정했다.
- 발신번호 `01025899695`가 SOLAPI에 사전등록되지 않아 국내 발송이 100% 거부된다. 한국은 전기통신사업법상 발신번호 사전등록이 의무이므로, 등록 전까지 어떤 코드 수정으로도 문자는 나가지 않는다. `delivered`가 처음부터 0건이었던 이유가 이것이다.
- 첫 재시도 5회는 새 코드 배포 전에 소진됐다(백오프 1+2+4+8분 = 15분). 진단을 위해 국내번호 2건만 `pending`으로 되돌렸고, 국제번호 2건은 결과가 달라지지 않으므로 두었다.
- `/api/notification-health`의 `oldestUnansweredAgeMinutes`가 미답변 67건에도 `null`을 반환하던 버그를 고쳤다. `getActiveConsultations()`가 원본 Firestore 문서를 반환해 `createdAt`이 Timestamp인데 `Date.parse()`를 적용해 NaN이 됐다.
- 백엔드 102개 테스트 통과.

## 2026-09-02 봇 생존 감시와 환자 답변 메일 병행 발송

- [x] `/poll` 호출마다 heartbeat를 기록한다 (60초 스로틀링, 봇은 약 10초 주기)
- [x] 폴링이 30분 이상 끊기면 메일로 경고하고, 복구되면 복구 알림을 보낸다
- [x] 장애 1건당 경고 1회만 발송해 야간 중단이 메일함을 채우지 않게 한다
- [x] `/api/notification-health`에 마지막 폴링 시각과 경과 분을 노출한다
- [x] 환자 답변 메일을 폴백이 아니라 다른 채널과 **동시에** 발송한다

### Review
- 봇 폰이 죽으면 큐가 조용해질 뿐 아무도 알 수 없었다. 이제 마지막 폴링 시각을 기록하고 30분 이상 끊기면 메일 경고가 간다. 임계값은 `BOT_HEARTBEAT_STALE_MINUTES`로 조정 가능하다.
- 봇이 약 10초마다 폴링하므로 매 호출을 기록하면 하루 약 9천 건의 쓰기가 된다. 60초 스로틀링으로 줄였고 테스트로 고정했다.
- 장애가 지속되는 동안 반복 경고하지 않는다. 야간에 폰이 꺼져 있으면 매 점검마다 메일이 갈 수 있어 장애당 1회로 제한했고, 복구 시 1회 알린다.
- heartbeat 기록이 실패해도 폴링 응답은 정상 진행한다. 알림 전달 경로가 관측 코드 때문에 막히면 안 된다.
- 환자 답변 메일이 카카오·SMS가 모두 실패했을 때만 나가던 구조를 바꿔, 동의한 이메일이 있으면 항상 함께 보낸다. 이 서비스의 채널들은 모두 한 번씩 조용히 실패한 전력이 있고, 환자가 답변 도착을 두 번 아는 것이 한 번도 모르는 것보다 낫다.
- 응답에 `notifiedChannels`를 추가해 어느 채널로 알렸는지 호출자가 확인할 수 있다.
- 백엔드 112개 테스트 통과.

## 2026-09-04 SOLAPI 발송 성공 및 의료진용 AI SOAP 요약 도입

- [x] SOLAPI 발신번호 등록 완료 후 실패했던 국내 2건을 재발송한다
- [x] 의료진 포털 전용 AI SOAP 요약을 생성한다 (환자 경로는 그대로 차단 유지)
- [x] 요약이 알림 메시지나 환자 응답에 절대 섞이지 않도록 테스트로 고정한다
- [x] 개인정보처리방침 수탁자 목록에 Google Gemini를 추가한다
- [x] Play 건강 앱 선언서에 AI 요약 범위와 재검토 항목을 명시한다
- [ ] `자동 임상 의사결정 지원: 아니요` 답변을 규제 검토로 확정한다 (제출 전 필수)
- [x] Play Data Safety 항목과 개인정보처리방침 수탁자 목록을 대조한다 (2026-09-14 대조, 불일치 2건 수정)

### Review
- 발신번호 등록 직후 재큐잉한 국내 2건이 `delivered`, `attemptCount=1`로 발송됐다. `patient_sms_notifications` 컬렉션 역사상 최초의 성공 발송이며, 원인이 `1062 발신번호 미등록`이었음이 확정됐다.
- AI 요약은 `services/doctorSummaryService.js`로 분리했다. `llmService.js`는 환자 경로가 모델 출력을 담지 않는다는 사실을 보증하는 파일이므로 섞지 않았다.
- 요약은 상담 문서의 `aiDoctorSummary` 필드에 저장되고 인증된 의료진 포털에서만 표시된다. 의료진 알림(카카오톡·이메일)은 여전히 건강정보를 담지 않는다. 알림 메시지·환자 응답에 요약이 섞이지 않는 것을 통합 테스트로 고정했다.
- 환자 응답 후 `setImmediate`로 생성한다. 모델 지연이 상담 접수를 늦추면 안 되고, 실패해도 상담은 정상 접수돼야 한다. 실제로 테스트가 이 결함을 잡아냈다(설정 누락 시 상담 접수가 500으로 실패). 스케줄러 전체를 try/catch로 감쌌다.
- 프롬프트에서 진단명 확정, 약물·용량·복용법, 검사 지시, 환자 전달 문장을 명시적으로 금지했다. 응급도 자동 분류도 하지 않는다.
- express의 `qs` 취약점 1건이 새로 보고돼 `6.16.0` override로 정리했다. `npm audit --omit=dev` 0건.
- 백엔드 120개 테스트 통과, 환자 앱 빌드 통과.

## 2026-09-04 보듬이 답변 초안 (승인제)

- [x] 환자 자동 답변 복원 요청을 검토하고 승인제로 범위를 확정한다
- [x] 보듬이가 SOAP 정리와 함께 **환자 답변 초안**을 생성하도록 확장한다
- [x] 초안은 의료진 포털에만 저장·표시하고 자동 전송 경로를 만들지 않는다
- [x] 개인정보처리방침과 Play 건강 앱 선언서에 범위를 반영한다

### Review
- 처음 요청은 `d78e4e9`에서 제거된 환자 자동 답변(AUTONOMOUS_REPLY) 복원이었다. 구현 도중 자동 승인 분류기가 해당 편집을 차단했고, 사용자와 범위를 다시 좁혀 **승인제**로 확정했다.
- 확정된 형태: 환자는 기존과 동일한 고정 접수 안내만 즉시 받는다. 보듬이는 의료진용 SOAP 정리와 답변 초안을 만들고, 의료진이 읽고 판단해 전송한다. 임상 판단과 발송 주체가 면허를 가진 의료진으로 유지된다.
- `llmService.js`는 원상복구했다. 이 파일은 "환자 경로에 모델 출력이 없다"를 보증하는 자리이며, 그 사실을 주석으로 명시했다.
- 초안은 `aiDoctorSummary.replyDraft`에 저장된다. 포털 상세 응답이 상담 문서를 그대로 반환하므로 별도 직렬화 변경은 필요 없다. 자동 전송 경로는 만들지 않았다 — 포털 답변 라우트가 유일한 발송자다.
- 프롬프트에서 진단명 확정, 약물·용량·복용법, 검사 지시에 더해 **응급 여부 단정**도 금지했다. 경증 판정은 가장 안전해 보이지만 틀렸을 때 결과가 가장 무겁다.
- 백엔드 127개 테스트 통과.
- 남은 사항: 의료법 검토는 여전히 제출 차단 항목 1번이다. 승인제는 판단 주체를 의료인으로 유지하지만, 확정은 변호사 검토로 해야 한다.

## 2026-09-06 알림 체계 복구 완료

- [x] 환자 답변 SMS 발송 (SOLAPI 발신번호 등록 후 최초 성공)
- [x] 의료진 메일 알림 (Resend HTTPS, `noreply@happydoctor.kr`, 받은편지함 도착 확인)
- [x] 카카오 봇 생존 감시
- [x] 미답변 집계 오류 수정 (실제 0건)
- [x] 보듬이 답변 초안 (승인제)
- [x] 보듬이 초안 실전 검증 — 실제 상담 접수 시 포털에 채워지는지 확인 (2026-09-14 확인)
- [ ] `_dmarc` TXT 레코드 추가 (선택, `v=DMARC1; p=none;`)
- [ ] 의료법 검토 (보듬이 승인제 구조 기준)
- [ ] D-U-N-S 무료 발급 가능 여부 확인 (PWA 유지 방침이라 비긴급)

### Review
- 시작점은 "알림이 안 와서 문의가 쌓였다"였다. 최종 확인된 원인은 서로 무관한 네 가지였다: 알림 목적지가 개인 카톡방 한 곳뿐, SMS 발신번호 미등록으로 6월 이후 delivered 0건, Render의 아웃바운드 SMTP 차단, 그리고 미답변 집계 코드의 필터 값 불일치.
- 마지막 항목은 내 버그였다. `'pending'`이 어느 분기에도 걸리지 않아 전체 상담이 미답변으로 집계됐고, 실제 0건인 상황을 68건으로 사흘간 보고했다.
- 메일 경로 진단에서 결론을 세 번 뒤집었다. 무한 대기 → 포트 차단 단정 → IPv6 오류 보고 → 라우팅 문제 단정 → 주소별 실측 → 포트 차단 확정. 오류 메시지 하나가 마지막 시도만 보여준다는 것을 늦게 인지했다.
- 그 과정에서 만든 관측 경로(`/api/version`의 채널 상태, `/api/notification-health`의 큐·백로그·봇 하트비트, `?verify=1`의 자격증명 검증과 주소별 프로브, 테스트 발송 라우트)가 이후 모든 오진을 잡아냈다. 침묵을 없애는 것이 개별 수정보다 값어치가 컸다.
- 백엔드 131개 테스트 통과, `npm audit --omit=dev` 0건.

## 2026-09-14 보듬이 초안 실전 검증 및 Play Data Safety 대조

- [x] 라이브 Firestore에서 `aiDoctorSummary` 생성 실적을 확인한다
- [x] 모델 경로(SOAP + replyDraft)를 프로덕션 부작용 없이 직접 검증한다
- [x] 의료진 포털이 초안을 실제로 표시하는지 확인한다 → **표시하지 않음, 미구현 확인**
- [x] Play Data Safety 항목과 개인정보처리방침 수탁자 목록을 대조한다 (제출 차단 5번)
- [x] 대조에서 드러난 불일치 2건을 수정한다
- [x] 의료진 포털에 보듬이 SOAP 요약·답변 초안 표시 UI를 구현한다 (미구현 상태라 기능이 도달하지 않음)
- [ ] Gemini API 키 요금 등급을 확인한다 (무료 등급이면 입력이 제품 개선에 사용될 수 있음)
- [x] 개인정보처리방침 수정분을 Vercel에 배포한다 (라이브에서 Resend·개정일 확인)

### Review

**보듬이 초안 — 생성은 되지만 아무도 볼 수 없다.**

- 라이브 Firestore 최근 상담 25건을 조회했다. `aiDoctorSummary`가 붙은 문서는 **단 1건**(2026-09-05)이고, 그마저 `replyDraft`가 없다. 답변 초안 기능이 배포된 것은 2026-09-06(`fa4a4da`)이므로 시점상 당연하다. **9/5 이후 신규 웹 상담이 0건이라 초안 경로는 운영에서 한 번도 실행된 적이 없다.**
- 실제 상담을 만들면 의료진 알림·환자 문자가 함께 나가므로, 대신 합성 문진 데이터로 `doctorSummaryService.generateSafely()`를 직접 호출했다. DB 쓰기도 알림도 없는 경로다. 결과: `status: ready`, SOAP 584자, replyDraft 647자, 5.7초. `09b244d`의 `thinkingBudget: 0` 수정 이후 JSON이 잘리지 않고 두 필드가 모두 채워진다.
- 프롬프트 금지 규칙도 출력에서 지켜졌다. 진단 확정 없음, 약물·용량 없음, 검사 지시 없음, 응급 단정 없음. A 항목은 감별 방향만 서술하고 P 항목은 추가 질문 목록만 제시했다.
- **그러나 `frontend/portal` 전체에 `aiDoctorSummary`나 `replyDraft`를 참조하는 코드가 한 줄도 없다.** `lib/api.ts`의 `Consultation` 타입에도 없다. 포털 상세 API(`GET /api/portal/consultations/:id`)가 상담 문서를 통째로 반환하므로 데이터는 응답에 실려 가지만, 화면이 그 필드를 읽지 않는다.
- 2026-09-04 Review에 "인증된 의료진 포털에서만 표시된다"고 적었는데, 정확히는 **표시할 화면을 만들지 않았다.** 직렬화가 필요 없다는 판단은 맞았지만 렌더링이 필요 없다고 넘어간 것이 잘못이다. 생성·저장·차단(환자 경로 격리)은 전부 테스트로 고정돼 있고, 유일하게 테스트가 없던 마지막 한 칸이 비어 있었다.
- replyDraft 본문이 "안녕하세요, 인턴 닥터 보듬입니다"로 시작한다. 이 글은 의료진이 검토해 자기 이름으로 보내는 초안이므로 발신 주체 표기가 어긋난다. UI 구현 시 프롬프트에서 자기소개 문장을 빼야 한다.

**Data Safety 대조 — 불일치 2건.**

- `개인정보 > 이메일 주소`가 Data Safety `선택하지 않을 유형`에 남아 있었다. 실제로는 상담 폼에서 동의 시 수집해 저장하고 답변 알림 메일을 보낸다. 개인정보처리방침 2번에는 이미 수집 항목으로 적혀 있어, **방침이 맞고 Play 선언이 틀린** 형태였다. 표에 행을 추가하고 제외 목록에서 뺐다.
- 개인정보처리방침 수탁자 목록이 메일 발송 주체를 `Google Gmail(SMTP)`로 고지하고 있었다. 2026-09-04에 Render의 SMTP 차단으로 Resend HTTPS로 전환했으므로 **공개 문서가 존재하지 않는 위탁 관계를 고지**하는 상태였다. `Resend`로 교체하고 갱신일을 명시했다.
- 대조 기록은 `docs/google-play/data-safety-ko.md`에 남겼고, Gemini API 키 등급 확인을 제출 차단 6번으로 추가했다. 무료 등급이면 입력이 제품 개선에 쓰일 수 있어 민감정보를 보내는 현 구조의 수탁 처리 전제가 무너진다.

## 2026-09-14 의료진 포털 보듬이 정리 표시 UI

- [x] 상담 상세 화면에 보듬이 SOAP 정리와 환자 답변 초안을 표시한다
- [x] 초안을 클립보드에 복사하는 버튼을 붙인다
- [x] 초안을 답변 입력란에 바로 넣는 버튼을 붙인다 (자동 전송 경로는 만들지 않는다)
- [x] 생성 실패한 상담에서도 화면이 깨지지 않게 한다
- [x] 답변 초안이 자기소개·서명을 쓰지 않도록 프롬프트를 고친다
- [x] 배포 후 실제 상담 1건으로 화면 표시를 확인한다 (사용자 확인)

### Review

- `AiDoctorSummarySection.tsx`로 분리했다. `page.tsx`가 이미 900줄에 가까웠고, 분리해야 목업 데이터로 화면을 직접 띄워볼 수 있었다.
- `lib/api.ts`의 `Consultation`에 `aiDoctorSummary`를 추가했다. 백엔드는 이전부터 이 필드를 응답에 실어 보내고 있었고, 타입에 없어서 화면이 쓰지 못하던 상태였다.
- 카드 색을 amber로 잡았다. 포털에서 blue는 의사용 번역본, emerald는 환자에게 실제 전달된 문장에 이미 쓰고 있다. AI 초안을 "환자에게 전달된 것"으로 오인하면 가장 위험하므로 두 색과 겹치지 않게 했다.
- 버튼은 두 개다. `복사`는 클립보드에 넣고 2초간 `복사됨`으로 바뀐다. `답변란에 넣기`는 답변 textarea에 채우고 포커스를 옮긴다. **자동 전송 경로는 만들지 않았다** — 포털 답변 라우트가 여전히 유일한 발송자다.
- 작성 중인 답변이 있으면 `window.confirm`으로 먼저 묻는다. 의료진이 쓰던 문장을 버튼 한 번에 날리면 안 된다.
- 클립보드는 `navigator.clipboard` 실패 시 `execCommand('copy')` 폴백을 탄다. 실패하면 `복사 실패`를 보여준다 — 조용히 아무 일도 안 일어나는 것이 가장 나쁘다.
- `status: 'failed'`이거나 두 필드가 모두 비면 중립 회색 카드로 "정리를 만들지 못했습니다"만 띄운다. 빈 amber 카드를 남기면 의료진이 초안을 기다리게 된다.
- 답변 초안이 "안녕하세요, 인턴 닥터 보듬입니다"로 시작하던 문제를 프롬프트에서 막았다. 의료진이 자기 이름으로 보내는 글이라 발신 주체가 어긋난다. 금지 문구를 테스트로 고정했다.
- 검증: 임시 프리뷰 라우트를 만들어 목업 데이터로 실제 컴포넌트를 브라우저에 띄웠다. 초안 삽입 시 textarea가 채워지고 포커스가 이동하는 것, 복사 버튼이 `복사됨`으로 바뀌는 것, 실패 상태 카드가 뜨는 것을 화면으로 확인한 뒤 프리뷰 라우트를 지웠다. 포털은 Google 로그인이 필요해 실제 상담 화면으로는 확인할 수 없다.
- 백엔드 136개 테스트 통과.

## 2026-09-14 보듬이 카드 배치 수정

- [x] 실제 상담 1건으로 라이브 확인 → 생성은 정상, 화면에서 못 찾음
- [x] 카드를 `환자 입력 원문` 다음, `SOAP 차트` 위로 올린다
- [x] 요약 도착 전 대기 카드를 띄우고 5초마다 재조회한다
- [x] 배포 후 사용자가 같은 상담에서 카드를 확인한다 (사용자 확인)

### Review

- 사용자가 테스트 상담을 넣고 "AI SOAP 차트가 안 나온다"고 했다. Firestore를 보니 **요약은 정상 생성돼 있었다** (`status: ready`, SOAP 425자, 초안 525자, 접수 6초 뒤 저장). 배포된 포털 번들에도 `보듬이 정리` 문자열이 들어 있었다. 기능은 동작했고 화면에서 찾을 수 없었을 뿐이다.
- 원인은 배치였다. 카드를 `답변 내역` 아래, `답변 전송` 바로 위에 뒀다. 초안을 답변창 옆에 붙이려는 의도였는데, 그 대가로 의료진이 차트를 여는 이유 자체를 화면 맨 밑에 묻었다. 사용자가 보낸 스크린샷은 `Follow-up 기록`에서 끊겨 있었다. **한 화면에 안 들어오는 위치는 없는 것과 같다.**
- 두 번째 구멍도 같이 막았다. 요약은 접수 6초 뒤에 저장되는데 의료진 알림은 접수 즉시 나간다. 알림을 보고 바로 열면 카드가 아예 없고, 상세 화면은 스스로 갱신하지 않았다. 이번에 사용자가 겪은 것이 이 경우였을 수도 있다 — 어느 쪽인지 확정할 근거는 없다. 대기 카드를 띄우고 요약이 올 때까지 5초마다 재조회하며, 접수 3분이 지나면 멈춘다.
- 검증은 이번에도 임시 프리뷰 라우트로 했다. 좁은 뷰포트(389px)에서 버튼이 줄바꿈되는 것까지 확인했다. 사용자가 모바일로 본다는 것을 스크린샷으로 알았기 때문이다.

## 2026-09-14 차트에서 상수 제거 및 동의 이메일 노출

- [x] `SOAP 차트`가 고정 안내문만 담는지 확인한다 (전체 122건 실측)
- [x] 고정 안내문을 `doctorChart`로 저장하지 않는다
- [x] 표준 접수 안내를 포털 응답에서 제외한다 (환자 상태 화면은 유지)
- [x] follow-up 로그의 고정 안내문도 저장하지 않는다
- [x] 알림 동의 이메일을 상세 화면에 표시한다
- [x] 배포 후 새 상담 1건으로 두 변경을 확인한다 (카카오 Fc2Cw0Ul)

### Review

- 사용자가 "SOAP 차트와 초기 답변 기록은 필요 없지 않나"고 물었다. 지우기 전에 122건을 실측했다: `doctorChart`는 **64건이 진짜 예진 노트**, 15건이 고정 안내문, 43건이 비어 있었고, `chatbotReply`는 **107건이 실제 답변**, 15건이 고정 문구였다. `d78e4e9` 이전 상담에는 내용이 있다. 섹션을 지웠다면 그 기록이 화면에서 사라졌을 것이다.
- 그래서 섹션이 아니라 **상수를 없앴다.** 고정 안내문은 애초에 `doctorChart`로 저장하지 않는다. 포털의 기존 `consultation.doctorChart ? ...` 가드가 그대로 숨겨주므로 프런트에 문자열 비교를 심을 필요가 없었다. 과거 문서는 손대지 않았다.
- 접수 안내는 사정이 다르다. 환자 상태 화면이 이 문구를 보여주므로 저장을 멈출 수 없다. 포털 상세 응답에서만 제외했고, 판단 기준(무엇이 상수인가)은 그 문자열이 정의된 백엔드에 뒀다. 번역본이 다르면 그대로 내려보낸다 — 외국어 환자가 실제로 무엇을 읽었는지는 상담별 정보다.
- 알림 동의 이메일은 보듬이 카드와 **같은 종류의 결함**이었다. 백엔드는 `email`·`normalizedEmail`을 저장하고 응답에도 실어 보내는데, `lib/api.ts` 타입에 필드가 없고 화면은 `.phone` 한 줄만 그렸다. 이메일만 동의한 환자는 연락처가 통째로 비어 보였다. 그 환자가 바로 국제번호 환자 — 국내 문자로는 닿지 않아 이메일이 유일한 수단인 사람이다.
- 백엔드 137개 테스트 통과, 포털 lint·build 통과.

## 2026-09-14 전달 실패 가시화 및 카카오 상담 보듬이 정리

- [x] 상태 확인 버튼이 같은 코드로도 다시 조회하게 한다
- [x] 답변 전송 결과에 실제 전달 채널을 표시한다
- [x] 전달 채널이 하나도 없으면 의료진에게 경고한다
- [x] 카카오 상담에도 보듬이 SOAP 정리·답변 초안을 생성한다
- [x] 두 접수 경로가 같은 스케줄러를 쓰도록 정리한다
- [x] 카카오 테스트 상담 1건으로 보듬이 정리 생성을 확인한다 (Fc2Cw0Ul: ready, SOAP 441자, 초안 479자)

### Review

**상태 확인 버튼.** 같은 URL로 이동해서가 아니라, 제출해도 `resolvedLookup`이 그대로라 어떤 effect도 재실행되지 않는 것이 원인이었다. 실제 조회는 `refreshKey`에 걸려 있는데 제출 핸들러가 건드리지 않았다. 입력란은 보고 있는 상담의 코드로 미리 채워지므로 **가장 흔한 경우가 정확히 아무 일도 안 하는 경우**였다. 제출마다 `refreshKey`를 올리고, 수동 확인은 1분 스로틀이 걸린 배경 알림 대신 로딩을 그대로 노출하며, 성공하면 확인 문구를 띄운다.

**전달 실패 가시화.** 백엔드는 어느 채널로 나갔는지 `notifiedChannels`를 계산해 응답에 담고 있었는데, 포털의 `postReply()`가 `Promise<void>`로 버리고 무조건 "답변이 전송되었습니다"를 띄웠다. **채널이 0개여도 성공으로 보였다.** 이 서비스의 채널은 전부 한 번씩 조용히 죽은 전력이 있고, 그때 의료진은 답변했다고 믿고 환자는 영원히 몰랐을 것이다. 이제 성공 시 `문자·이메일로 알렸습니다`처럼 실제 채널을 적고, 0개면 호박색 경고로 "환자가 상태 링크를 직접 열어야 볼 수 있다"고 알린다. 빈 배열이 응답에서 살아남는 것을 통합 테스트로 고정했다.

**카카오 상담 보듬이 정리.** `scheduleDoctorSummary()`가 웹 접수에만 붙어 있어 카카오 상담 33건은 SOAP 정리도 답변 초안도 없었다. 같은 포털에서 같은 의료진이 보는데 한쪽만 빈손이었다. 공통 로직을 `services/doctorSummaryScheduler.js`로 빼고 두 경로가 함께 쓰게 했다 — 라우트에 복사했다면 두 흐름이 또 갈라졌을 것이다. `doctorSummaryService`는 DB 의존성 없이 두기 위해 스케줄러를 별도 모듈로 분리했다.
- 스케줄러 단위 테스트 4건을 추가했다: 생성·저장, 실패해도 호출자에게 던지지 않음, 비활성/상담ID 없음일 때 무동작, 빈 요약은 저장하지 않음.
- 테스트 하네스에 함정이 있었다. 스케줄러가 require 시점에 `dbService`를 캡처해서, 캐시에 남으면 **이후 모든 테스트가 첫 테스트의 목을 물려받았다.** 기존 AI 요약 테스트가 이걸로 깨졌고, 라우트와 함께 스케줄러도 캐시에서 지우도록 고쳤다.
- 백엔드 142개 테스트 통과.

## 2026-09-14 카카오 상담 흐름 점검

- [x] 카카오에서 증상을 적었을 때 무슨 일이 일어나는지 추적한다
- [x] 인사말에 붙는 상태 링크가 어느 상담을 가리키는지 확인한다
- [x] 오래되거나 종료된 상담에는 링크를 붙이지 않는다
- [x] 인사말이 환자의 말을 인용하고 `상담 시작`으로 유도한다
- [x] 상태 페이지에 요청 타임아웃과 지연 안내를 넣는다
- [~] 오픈빌더 폴백 블록을 예진 흐름에 연결한다 → **2026-09-14 사용자 결정으로 하지 않음**

### Review

- 사용자가 카카오 채널에 "온 몸이 아파요"를 보냈더니 환영 인사와 상태 링크만 돌아왔다. 그 링크의 토큰을 조회하니 **2026-04-06 접수, 주호소 '흉통', 이미 의료진 답변 완료** 상담이었다. 5개월 전 건이다.
- 원인 두 가지가 겹쳤다. 첫째, 자유 문장은 폴백 블록 → `/kakao/check-doctor-reply`로 가고 이 핸들러는 **상담을 만들지 않는다.** 접수는 `상담 시작` → 오픈빌더 슬롯필링 → `/triage-complete` 경로뿐이다. 둘째, `getLatestConsultationTracking()`이 나이·종료 여부를 보지 않고 최신 1건을 반환해 그 링크가 붙었다.
- 인사말이 "먼저 말씀해 주세요"라고 해놓고, 말한 사람에게 아무것도 하지 않았다. 그 말은 저장되지도 전달되지도 않는다.
- 링크 자격 판정을 **식별자 발급보다 먼저** 했다. 기존에는 인사할 때마다 5개월 된 상담에 24시간짜리 조회 코드를 새로 발급하고 있었다. 레거시 `status: 'CLOSED'`는 `isConsultationClosed()`가 잡지 못해 별도로 처리했다.
- 상태 페이지가 비어 보인 건 별개였다. 같은 토큰이 일반 브라우저에서는 정상 렌더되고 API도 200이었다. 요청에 타임아웃·재시도·지연 안내가 없어 느린 백엔드와 고장난 페이지가 구분되지 않았다. 45초 타임아웃과 6초 지연 안내를 넣었다.
- 백엔드 146개 테스트 통과. 환자 앱 lint·build 통과.
- 자유 문장을 그대로 접수로 이어가는 안(오픈빌더 폴백 블록 연결)은 **2026-09-14 사용자 결정으로 진행하지 않는다.** 따라서 카카오 접수는 `상담 시작` 버튼 경로 하나로 유지되고, 폴백은 그쪽으로 안내하는 역할까지만 한다. 인사말이 증상을 인용하고 다시 적어 달라고 말하는 것이 이 구조에서 환자에게 줄 수 있는 최선이다.


## 2026-09-14 추가 질문 답변 초안

- [x] 환자의 추가 질문에 대한 답변 초안을 생성한다
- [x] 초진 SOAP을 덮어쓰지 않고 별도 필드에 저장한다
- [x] 포털에 질문과 초안을 함께 표시한다
- [x] 실제 모델로 초안 품질을 확인한다
- [x] 배포 후 실제 추가 질문 1건으로 확인한다 (6m1bNS5l: aiFollowUpDraft ready 154자)

### Review

- 첫 상담은 SOAP 정리와 답변 초안을 받는데, 추가 질문은 알림만 가고 초안이 없었다. 같은 의료진이 같은 상담에서 답하는데 한쪽만 빈손이었다.
- **SOAP은 다시 만들지 않는다.** 후속 질문이 왔다고 초진 차트가 달라지지 않는다. 의료진이 지금 하는 일은 새 질문 하나에 답하는 것이라 답변 초안만 만든다.
- `aiDoctorSummary`를 덮어쓰지 않고 `aiFollowUpDraft`에 따로 저장했다. 덮어썼다면 초진 SOAP을 잃는다. 최신 질문 하나만 유지한다 — 의료진이 답하는 것이 그것이다.
- 모델에는 기존 문진 + **가장 최근** 의료진 답변 + 새 질문을 넘긴다. 첫 답변이 아니라 마지막 답변이 맥락이다.
- 금지 항목은 초진 초안과 동일하고, 하나 더 넣었다: 이번에 물은 것에만 답하고 앞선 답변을 반복하지 말 것.
- 빈 질문 테스트가 결함을 잡았다. 공백만 있는 문자열은 truthy라 가드를 통과해 상담 조회와 모델 호출까지 갔다가 아무것도 못 만들고 끝났다. 가드에서 `trim`하도록 고쳤다.
- 실측: 이전 답변이 있는 상태에서 "체온은 37.8도였고 소변볼 때 따끔거려요. 병원에 꼭 가야 할까요?" → 310자 초안. 새 정보에만 답하고, 진단을 확정하지 않으며, 병원에 가야 할 변화를 알려준다.
- 백엔드 148개 테스트 통과. 포털 lint·build 통과. 삽입·복사·실패 상태를 화면으로 확인했다.

## 2026-09-14 대화형 보기 (포털 · 환자 상태 화면)

- [x] 포털의 `Follow-up 기록`과 `답변 내역`을 하나의 대화 타임라인으로 합친다
- [x] 환자 상태 화면에도 전체 대화를 시간 순으로 표시한다
- [x] 공개 상태 API가 환자의 추가 질문 본문을 내려보낸다
- [x] 배포 후 실제 상담에서 두 화면을 확인한다 (사용자 확인)

### Review

- 포털은 환자 질문과 의료진 답변이 **서로 다른 섹션**에 있어서, 누가 무엇을 언제 말했는지 알려면 두 목록을 오가며 시각을 눈으로 맞춰야 했다. 하나의 `대화 내역`으로 합치고 시간 순으로 정렬했다. 보는 사람이 오른쪽에 온다 — 포털에서는 의료진, 환자 화면에서는 환자.
- 첫 말풍선은 접수 시 주호소다. 없으면 대화가 중간부터 시작한다.
- `ESCALATE`, `자동 종료` 같은 비질문 로그는 **가운데 작은 회색 줄**로 남겼다. 지우면 대화가 비어 있는 구간이 왜 생겼는지 설명되지 않는다.
- 환자 상태 화면은 **최신 답변 1건만** 보여주고 있었다. 추가 질문을 한 환자가 자기 질문도, 그 전 답변도 볼 수 없었다. 전체 스레드로 바꿨다.
- 이를 위해 공개 상태 payload에 `patientQuestions`를 추가했다. **환자에게는 본인이 쓴 원문**을 돌려준다 — 로그의 한국어 번역본은 의료진용이고, 외국어로 쓴 환자에게 그걸 보여주면 자기 말이 고쳐져 돌아온 것처럼 읽힌다.
- 합치면서 `FollowUpItem`, `followUpActionLabel`, `hasDeliveredTranslation`이 호출부를 잃어 제거했다.
- 백엔드 150개 테스트 통과. 두 앱 클린 빌드 통과.
- **빌드 확인에서 한 번 미끄러졌다.** 지운 프리뷰 라우트의 생성 타입이 `.next` 캐시에 남아 포털 빌드가 실패했는데, `| tail`이 종료코드를 가리고 감시 필터가 `Failed to compile|Type error`만 보고 있어 "통과"로 읽힐 뻔했다. 실제 실패 문구는 `Next.js build worker exited with code: 1`이었다. `.next`를 지우고 클린 빌드로 확인했다.

## 2026-09-14 대화창 하단 입력 (양쪽 화면)

- [x] 환자 상태 화면의 추가 질문 입력을 대화 카드 하단으로 옮긴다
- [x] 의료진 포털의 답변 전송 입력을 대화 카드 하단으로 옮긴다
- [x] 모바일 폭에서 입력창이 쓸 만한 크기가 되게 한다
- [x] 배포 후 두 화면을 실제 상담으로 확인한다 (사용자 확인)

### Review

- 두 화면 모두 대화는 한 곳에, 쓰는 곳은 다른 곳에 있었다. 환자 화면은 옆 카드(`다음 행동`), 포털은 아래 별도 섹션(`답변 전송`)이었다. 사용자가 쓰는 모든 메신저에서 입력은 대화 바로 아래 붙어 있다.
- 두 스레드 컴포넌트에 `composer` 슬롯을 만들고 각 입력 폼을 그 안으로 옮겼다. 폼의 상태와 제출 로직은 그대로 두고 JSX 위치만 옮겼다 — 동작을 건드리지 않고 배치만 바꾸는 것이 안전하다.
- `StatusCloseActions`에서 추가 질문 폼을 `StatusFollowUpComposer`로 분리했다. 종료 버튼은 그대로 두고, 입력만 대화 카드 안으로 들어갔다.
- 메시지 목록을 `max-h-[60vh]`로 제한하고 최신 메시지로 스크롤한다. 대화가 길어져도 입력창이 화면 밖으로 밀리지 않는다.
- 389px 폭에서 입력창과 버튼을 나란히 두니 글상자가 너무 좁았다. 좁은 화면에서는 세로로 쌓고, `sm` 이상에서만 나란히 놓는다.
- 두 앱 클린 빌드 통과. 이번에는 `| tail` 없이 종료코드를 직접 확인했다.

## 2026-09-14 종료 상태 판정 통일

- [x] 닫힘 판정이 흩어진 지점을 전부 찾는다 (8곳)
- [x] 판정 규칙을 한 모듈로 모으고 레거시 `CLOSED`를 인정한다
- [x] 포털 종료 탭 조회가 두 표기를 모두 찾게 한다
- [x] 공개 통계도 같은 규칙으로 센다
- [x] 배포 후 포털 종료 탭에서 옛 상담이 보이는지 확인한다 (운영 데이터로 종료 탭 17 → 28건 확인, 화면 확인은 미실시)

### Review

- 문제는 값이 세 가지라는 것이 아니라 **판정이 8곳에 각각 적혀 있었다는 것**이다. 전부 `status === 'COMPLETED' || closedAt`이라 이 서비스가 예전에 쓰던 `CLOSED`를 놓쳤고, 그래서 같은 상담이 어떤 경로에서는 닫힌 것으로, 어떤 경로에서는 열린 것으로 취급됐다.
- 포털 조회가 `COMPLETED`만 찾아서 **26건이 어느 탭에도 없었다.** 미답변도, 답변 완료도, 종료도 아니었다. 종료 탭이 17 → 28건이 됐다.
- 규칙을 `services/consultationStatus.js`로 뺐다. dbService에 두면 **DB를 목으로 바꾼 테스트가 판정 함수까지 잃는다** — 실제로 그렇게 4개가 깨져서 설계를 바로잡았다. 판정은 DB가 필요 없는 순수 함수다.
- 앞서 두 번 `|| data.status === 'CLOSED'`를 손으로 덧붙였던 자리도 이 규칙으로 정리했다. 규칙이 한 곳이면 기억할 필요가 없다.
- **데이터는 그대로 뒀다.** 26건에 `closedAt`을 채우려면 기록된 적 없는 종료 시각을 지어내야 하고, 그 상담들이 가진 상태값은 실제로 그렇게 닫혔다는 참된 기록이다. 지금은 어느 경로도 `CLOSED`를 새로 쓰지 않으므로 이건 현행 포맷이 아니라 이력이다.
- 백엔드 159개 테스트 통과 (닫힘 규칙 5건 추가: 두 표기, closedAt 우선, 열린 상담, 모르는 상태는 열림으로, Firestore `in` 쿼리에 쓸 수 있는 형태).
