...
- [ ] Render에 `SMTP_USER`/`SMTP_PASS`를 설정한다 (사용자 직접 작업)
- [ ] 의료진 단톡방에서 `~알림방등록`을 보내 단톡방 알림을 복구한다 (사용자 직접 작업)
- [ ] Render에 SOLAPI 3종 변수를 설정해 환자 SMS 24건을 살린다 (사용자 직접 작업)
...
- [ ] MessengerBot 폰의 `lastPolledAt`을 기록하고 폴링 중단 시 메일 경고를 보낸다 (3단계)
...
- [ ] SOLAPI에 발신번호 `01025899695`를 사전등록한다 (사용자 직접 작업, 소유 증명 필요)
...
- [ ] Play Data Safety 항목과 개인정보처리방침 수탁자 목록을 대조한다 (제출 전 필수)
...
- [x] 보듬이 답변 초안 (승인제)
- [ ] 보듬이 초안 실전 검증 — 실제 상담 접수 시 포털에 채워지는지 확인
...
- [ ] 카카오 테스트 상담 1건으로 보듬이 정리 생성을 확인한다
...
- [x] 공개 통계도 같은 규칙으로 센다
- [ ] 배포 후 포털 종료 탭에서 옛 상담이 보이는지 확인한다
