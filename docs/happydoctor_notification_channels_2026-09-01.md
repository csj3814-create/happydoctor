...
국내번호가 전부 실패하는 흔한 원인은 발신번호 사전등록 미완료(코드 3037)입니다.
...
다만 **발송 자체는 아직 성공하지 못하고 있습니다.** SOLAPI 콘솔에서 발신번호
`01025899695`의 사전등록 승인 상태와 계정 잔액을 확인해야 합니다. 계정 접근이 어렵다면
다음 발송 실패 시 `lastFailureReason`에 남는 SOLAPI 상태코드로 원인을 특정할 수 있습니다.
