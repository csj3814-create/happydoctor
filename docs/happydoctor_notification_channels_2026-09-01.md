...
### 1-3-1. 차단 해소 후 드러난 발송 실패 원인 (2026-09-02)

큐를 뚫자 4건 모두 시도됐고, 두 종류의 실패가 나왔습니다.

**국제번호 (`+92`, `+61`)** — SOLAPI는 한국 국내 문자 서비스이고 발신번호도 국내번호라
해외 수신자에게 보낼 수 없습니다. SDK가 페이로드 검증에서 거부합니다.
해피닥터는 외국인 이용자가 많으므로 **SMS로는 구조적으로 도달할 수 없는 환자군이 존재합니다.**
이들에게는 이메일이 유일한 대안입니다.

**국내번호** — SOLAPI가 접수 후 거부하며 `failedMessageList`를 확인하라고 안내합니다.
상세 코드를 저장하지 않아 원인 미상이었으므로, 2026-09-02에 `statusCode`/`statusMessage`를
기록하도록 고쳤습니다. 다음 실패부터 `lastFailureReason`에 실제 사유가 남습니다.
**2026-09-02 확인 결과: `1062 발신번호 미등록`.** 발신번호 `01025899695`가 SOLAPI에
사전등록되어 있지 않습니다. 한국은 전기통신사업법상 발신번호를 사전등록·검증해야 문자를
보낼 수 있으므로, 등록 전까지 국내 발송은 100% 거부됩니다. `delivered`가 처음부터
0건이었던 이유입니다.
...
(재시도 5회 상한, 지수 백오프, 죽은편지함, 배치 중단 방지, 국제번호 즉시 포기,
실패 사유 상세 기록과 300자 제한).

다만 **발송 자체는 아직 성공하지 못합니다.** 원인은 `1062 발신번호 미등록`으로 확정됐습니다.
SOLAPI 콘솔에서 발신번호 `01025899695`를 등록해야 합니다.

발신번호 등록에는 해당 번호의 소유 증명이 필요합니다 (통신서비스 이용증명원 또는 ARS 인증).
번호 명의자가 직접 진행해야 하며, 단체 명의 번호로 등록하는 방법도 있습니다.
등록이 완료되기 전까지 국내 환자에게 문자는 나가지 않습니다.
