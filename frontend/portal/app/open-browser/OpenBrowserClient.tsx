...
function buildAndroidChromeIntentUrl(targetUrl: string) {
  const parsed = new URL(targetUrl)
  const scheme = parsed.protocol.replace(':', '')
  const pathWithQuery = `${parsed.pathname}${parsed.search}${parsed.hash}`
  return `intent://${parsed.host}${pathWithQuery}#Intent;scheme=${scheme};package=com.android.chrome;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;end`
}
...
function isLikelyInAppBrowser(userAgent: string) {
  return /KAKAOTALK|KAKAOSTORY|DaumApps|FBAN|FBAV|Instagram|NAVER|; wv\b|WebView/i.test(userAgent)
}
...
export default function OpenBrowserClient({ nextPath }: OpenBrowserClientProps) {
  ...
  const chromeIntentUrl = useMemo(() => buildAndroidChromeIntentUrl(targetUrl), [targetUrl])
  ...
  useEffect(() => {
    ...
    const inAppBrowser = isLikelyInAppBrowser(userAgent) || isKakaoInAppBrowser(userAgent)
  ...
  }, [chromeIntentUrl, externalUrl, targetUrl])
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#eff7ff_0%,#ffffff_48%,#f7fbff_100%)] px-5 py-10 text-slate-900">
      <div className="mx-auto max-w-lg rounded-[2rem] border border-slate-200 bg-white/95 p-6 shadow-[0_24px_60px_rgba(15,47,89,0.10)] sm:p-8">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-sky-700">기본 브라우저 열기</p>
        ...
        <div className="mt-6 rounded-[1.5rem] bg-slate-50 p-4 text-sm leading-7 text-slate-600">
          ...
          <ol className="mt-3 list-decimal space-y-1 pl-5">
            <li>아래 버튼으로 크롬 또는 기본 브라우저에서 다시 열어 주세요.</li>
        ...
        <p className="mt-5 text-xs leading-6 text-slate-400">
          {inAppBrowser
            ? autoOpenAttempted
              ...
              : '잠시 후 외부 브라우저 열기를 자동으로 시도합니다.'
            : '이미 기본 브라우저라면 잠시 후 포털로 바로 이동합니다.'}
        </p>
      </div>
    </main>
  )
