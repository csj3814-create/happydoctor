...
function cardTone(consultation: Consultation): string {
  ...
  if (followUpCount(consultation) > 0) return 'border-amber-300 bg-amber-50/50 hover:border-amber-400'
        ...
        <div className="mt-6 rounded-2xl bg-zinc-50 px-5 py-4">
          <div className="space-y-2 text-sm text-zinc-600">
            <p>
              ...
              <br />
              {authStatus.doctor.email}
            </p>
            <p>
              <span className="font-semibold text-zinc-800">승인 요청 시각</span>
              <br />
              {formatDateTime(authStatus.request?.requestedAt)}
            </p>
            <p>
              <span className="font-semibold text-zinc-800">최근 로그인 시각</span>
              <br />
              {formatDateTime(authStatus.request?.lastLoginAt)}
            </p>
          </div>
        </div>
...
export default function HomePage() {
  ...
  }, [user, portalAuth?.accessStatus, tab, deferredSearch, currentPage, refreshKey])
  ...
  }, [user, portalAuth?.accessStatus, tab, refreshKey])
      ...
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4">
        <div className="flex w-full max-w-sm flex-col items-center gap-6 rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
          <div className="space-y-2 text-center">
            ...
            <p className="text-sm text-zinc-500">Google 계정으로 로그인한 뒤 대표자 승인 후 상담에 참여할 수 있습니다.</p>
          </div>
  ...
  if (portalAuth && portalAuth.accessStatus === 'pending') {
    return (
      <PendingApprovalView
        authStatus={portalAuth}
        onRefresh={loadPortalAccessStatus}
      ...
      <header className="sticky top-0 z-10 border-b border-zinc-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between gap-4 px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="hidden max-w-[200px] truncate text-xs text-zinc-500 sm:block">
              {user.email}
            </span>
            <button
          ...
          <>
            <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-2xl border border-green-200 bg-green-50/60 px-4 py-4 shadow-sm">
                ...
                <p className="mt-1 text-sm text-zinc-500">의료진 회신이 전달된 상담</p>
              </div>
                            ...
                            <div className="min-w-0 space-y-2">
                              <div className="flex flex-wrap items-center gap-2">
                                <span className="text-sm font-semibold text-zinc-800">
                                  {consultation.patientData.age} / {genderLabel(consultation.patientData.gender)}
                                </span>
