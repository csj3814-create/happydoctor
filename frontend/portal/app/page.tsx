...
type ConsultationStage = 'pending' | 'followup' | 'replied' | 'closed'

const PAGE_SIZE = 12
const EMPTY_SUMMARY: ConsultationSummary = { pending: 0, replied: 0, closed: 0, followUp: 0 }
const TAB_TO_STATUS: Record<InboxTab, 'active' | 'followup' | 'replied' | 'closed'> = {
  pending: 'active',
  followup: 'followup',
  replied: 'replied',
  closed: 'closed',
...
function genderLabel(gender: string) {
  ...
  if (gender === 'prefer_not_to_say') return '밝히지 않음'
  return gender || '미상'
}
...
function latestFollowUpTimestampMs(consultation: Consultation) {
  return (consultation.followUpLogs ?? [])
    .map((log) => timestampMs(log.timestamp))
    .reduce((latest, current) => Math.max(latest, current), 0)
}

function hasPendingFollowUp(consultation: Consultation) {
  const latestFollowUp = latestFollowUpTimestampMs(consultation)
  if (!latestFollowUp) return false
  return latestFollowUp > timestampMs(consultation.doctorRepliedAt)
}

function categorize(consultation: Consultation): ConsultationStage {
  if (consultation.status === 'COMPLETED' || consultation.closedAt) return 'closed'
  if (hasPendingFollowUp(consultation)) return 'followup'
  if (consultation.doctorRepliedAt) return 'replied'
  return 'pending'
}

function followUpCount(consultation: Consultation) {
  return consultation.followUpLogs?.length ?? 0
}

function latestFollowUpTimestamp(consultation: Consultation) {
  const latest = latestFollowUpTimestampMs(consultation)
  return latest ? new Date(latest).toISOString() : null
}

function cardTone(consultation: Consultation) {
  const category = categorize(consultation)
  if (category === 'closed') return 'border-zinc-200 hover:border-zinc-300'
  if (category === 'followup') return 'border-amber-300 bg-amber-50/50 hover:border-amber-400'
  if (category === 'replied') return 'border-green-200 bg-green-50/40 hover:border-green-300'
  return 'border-zinc-200 hover:border-zinc-300'
}

function statusBadge(consultation: Consultation) {
  const category = categorize(consultation)
  if (category === 'closed') return { label: '상담 종료', className: 'bg-zinc-100 text-zinc-600' }
  if (category === 'followup') return { label: 'Follow-up', className: 'bg-amber-100 text-amber-800' }
  if (category === 'replied') return { label: '답변 완료', className: 'bg-green-100 text-green-700' }
  return { label: '답변 대기', className: 'bg-red-100 text-red-700' }
}

function complaintForDoctor(consultation: Consultation) {
  return consultation.translatedPatientDataKo?.cc || consultation.patientData.cc
}

function symptomForDoctor(consultation: Consultation) {
  return consultation.translatedPatientDataKo?.symptom || consultation.patientData.symptom
}
...
function SummaryShortcutCard({
  ...
  return (
    <div className={`rounded-2xl border px-4 py-4 shadow-sm ${cardClassName}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className={`text-xs font-semibold uppercase tracking-[0.18em] ${labelClassName}`}>{label}</p>
        ...
        <button
          ...
          className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition disabled:cursor-default disabled:opacity-60 ${buttonClassName}`}
        >
          {buttonDisabled ? '현재 탭' : '바로가기'}
        </button>
      </div>
    </div>
  )
  ...
  return (
    <section className="rounded-2xl border border-blue-200 bg-blue-50/70 p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-700">대표자 승인</p>
          <h2 className="mt-2 text-lg font-semibold text-zinc-900">승인 대기 중인 의료진</h2>
          <p className="mt-1 text-sm text-zinc-600">
            새로 로그인한 의료진은 여기서 승인하면 다음 로그인부터 바로 차트에 참여할 수 있습니다.
          </p>
        </div>
        <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-blue-700 shadow-sm">
          {pendingRequests.length}명 대기
        </span>
      </div>

      {pendingRequests.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-dashed border-blue-200 bg-white/85 px-4 py-5 text-sm text-zinc-500">
          현재 승인 대기 중인 의료진이 없습니다.
        </div>
      ) : (
        <ul className="mt-4 space-y-3">
          {pendingRequests.map((request) => (
            <li
              key={request.email}
              className="rounded-2xl border border-white/70 bg-white px-4 py-4 shadow-[0_10px_24px_rgba(19,78,152,0.08)]"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-zinc-900">{request.name || request.email}</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-400">
                    <span>요청 시각 {formatDateTime(request.requestedAt)}</span>
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4 py-10">
      <div className="w-full max-w-xl rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-700">승인 대기</p>
        <h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-zinc-900">
          대표자 승인 후 포털을 사용할 수 있습니다
        </h1>
        <p className="mt-4 text-sm leading-7 text-zinc-600">
          {authStatus.doctor.name || authStatus.doctor.email} 의료진 계정이 승인 대기 상태로 등록되었습니다.
          대표자가 포털에서 승인하면 다음부터 상담 차트 확인과 답변이 가능합니다.
        </p>
        <div className="mt-6 rounded-2xl bg-zinc-50 px-5 py-4">
          <div className="space-y-2 text-sm text-zinc-600">
            <p><span className="font-semibold text-zinc-800">이메일</span><br />{authStatus.doctor.email}</p>
            <p><span className="font-semibold text-zinc-800">승인 요청 시각</span><br />{formatDateTime(authStatus.request?.requestedAt)}</p>
            <p><span className="font-semibold text-zinc-800">최근 로그인 시각</span><br />{formatDateTime(authStatus.request?.lastLoginAt)}</p>
          </div>
        </div>
        <div className="mt-6 flex flex-wrap gap-3">
          <button onClick={onRefresh} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-700">
            승인 상태 다시 확인
          </button>
          <button onClick={onLogout} className="rounded-xl border border-zinc-200 bg-white px-5 py-3 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-50">
            로그아웃
          </button>
        </div>
      </div>
...
export default function HomePage() {
  ...
  async function loadPortalAccessStatus() {
    ...
    try {
      const nextStatus = await getPortalAuthStatus()
      setPortalAuth(nextStatus)
      setPendingRequests(nextStatus.pendingRequests || [])
      setError(null)
    } catch (fetchError) {
      setPortalAuth(null)
      setPendingRequests([])
      setError(fetchError instanceof Error ? fetchError.message : '포털 권한 상태를 불러오지 못했습니다.')
    } finally {
  ...
  useEffect(() => {
    if (!user || portalAuth?.accessStatus !== 'approved') return
    let cancelled = false

    async function loadSummary() {
      try {
        const [summaryData, stats] = await Promise.all([fetchConsultationSummary(), getMyHDT()])
        if (cancelled) return
  ...
  }, [user, portalAuth?.accessStatus, refreshKey])

  useEffect(() => {
    if (!user || portalAuth?.accessStatus !== 'approved' || tab === 'leaderboard') return
    let cancelled = false
  ...
  }, [currentPage, deferredSearch, portalAuth?.accessStatus, refreshKey, tab, user])
  ...
  }, [portalAuth?.accessStatus, refreshKey, tab, user])
  ...
  async function handleApprove(email: string) {
    ...
    setError(null)
    try {
      const payload = await approveDoctorRequest(email)
      setPendingRequests(payload.pendingRequests || [])
    } catch (approveError) {
      setError(approveError instanceof Error ? approveError.message : '의료진 승인 처리에 실패했습니다.')
    } finally {
...
진행할까요?`,
  ...
  const counts = { pending: summary.pending, followup: summary.followUp, replied: summary.replied, closed: summary.closed }
  const totalPages = Math.max(1, Math.ceil(totalConsultations / PAGE_SIZE))
  const isApprovedDoctor = portalAuth?.accessStatus === 'approved'

  if (authLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-50">
        <p className="text-sm text-zinc-500">권한 상태를 확인하고 있습니다...</p>
      </div>
    )
  }
        ...
        <div className="flex w-full max-w-sm flex-col items-center gap-6 rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
          <div className="space-y-2 text-center">
            ...
            <p className="text-sm text-zinc-500">
              Google 계정으로 로그인한 뒤 대표자 승인 후 상담에 참여할 수 있습니다.
            </p>
          </div>
          {error ? (
            <p className="w-full rounded-xl bg-red-50 px-4 py-3 text-center text-sm text-red-600">{error}</p>
          ) : null}
  ...
  if (accessLoading && !portalAuth) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-50">
        <p className="text-sm text-zinc-500">승인 상태를 확인하고 있습니다...</p>
      </div>
    )
  }

  if (portalAuth && portalAuth.accessStatus === 'pending') {
    return <PendingApprovalView authStatus={portalAuth} onRefresh={loadPortalAccessStatus} onLogout={handleLogout} />
  }
      ...
      <header className="sticky top-0 z-10 border-b border-zinc-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between gap-4 px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="hidden max-w-[200px] truncate text-xs text-zinc-500 sm:block">{user.email}</span>
            <button
              onClick={() => setRefreshKey((value) => value + 1)}
              disabled={listLoading || boardLoading || accessLoading}
          ...
          <>
            <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <SummaryShortcutCard
                ...
                description="직접 회신이 필요한 상담"
                cardClassName="border-zinc-200 bg-white"
                labelClassName="text-zinc-400"
                buttonClassName="border border-zinc-200 bg-zinc-50 text-zinc-600 hover:bg-zinc-100"
                buttonDisabled={tab === 'pending'}
                onShortcut={() => setTab('pending')}
              />
              <SummaryShortcutCard
                label="Follow-up"
                count={summary.followUp}
                description="최근 follow-up 이후 답변이 필요한 상담"
                cardClassName="border-amber-200 bg-amber-50/70"
                labelClassName="text-amber-700"
                buttonClassName="border border-amber-200 bg-white/80 text-amber-800 hover:bg-white"
                buttonDisabled={tab === 'followup'}
                onShortcut={() => setTab('followup')}
              />
              <SummaryShortcutCard
                label="답변 완료"
                count={summary.replied}
                description="의료진 회신이 전달된 상담"
                cardClassName="border-green-200 bg-green-50/60"
                labelClassName="text-green-700"
                buttonClassName="border border-green-200 bg-white/80 text-green-800 hover:bg-white"
                buttonDisabled={tab === 'replied'}
                onShortcut={() => setTab('replied')}
              />
              <SummaryShortcutCard
                label="종료 상담"
                count={summary.closed}
                description="종료 사유까지 확인 가능한 상담"
                cardClassName="border-zinc-200 bg-white"
                labelClassName="text-zinc-400"
                buttonClassName="border border-zinc-200 bg-zinc-50 text-zinc-600 hover:bg-zinc-100"
                buttonDisabled={tab === 'closed'}
                onShortcut={() => setTab('closed')}
              />
            </section>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
                <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-xs text-zinc-400">증상, 번역된 요약, 나이, 환자 ID로 빠르게 찾을 수 있습니다.</p>
                  </div>
            ...
            {tab === 'leaderboard' ? (
              boardLoading ? (
                <div className="flex items-center justify-center py-20">
                  <p className="text-sm text-zinc-400">리더보드를 불러오는 중입니다...</p>
                </div>
              ) : (
                <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                  <div className="border-b border-zinc-100 px-5 py-4">
                    <p className="text-xs text-zinc-500">답변 1건당 100 HDT, 환자 확인 시 50 HDT가 지급됩니다.</p>
                  </div>
                  {leaderboard.length === 0 ? (
                    <div className="flex items-center justify-center py-16">
                      <p className="text-sm text-zinc-400">아직 집계가 시작되지 않았습니다.</p>
                    </div>
                  ) : (
                    <ul className="divide-y divide-zinc-100">
                      {leaderboard.map((doctor, index) => (
                        ...
                        >
                          ...
                          <span className="text-sm font-bold text-amber-600">{doctor.hdt.toLocaleString()} HDT</span>
                        </li>
                      ))}
                    </ul>
                  )}
              ...
              <div className="flex items-center justify-center py-20">
                <p className="text-sm text-zinc-400">상담 목록을 불러오는 중입니다...</p>
              </div>
              ...
              <>
                <ul className="flex flex-col gap-3">
                  {consultations.map((consultation) => {
                    ...
                    const category = categorize(consultation)
                    const latestFollowUpAt = latestFollowUpTimestamp(consultation)
                    const secondaryTimestamp = tab === 'followup' && latestFollowUpAt
                      ? latestFollowUpAt
                      : category === 'replied'
                    ...
                    const secondaryLabel = tab === 'followup' && latestFollowUpAt
                      ? '최근 Follow-up'
                      : category === 'replied'
                        ? '최근 회신'
                        : category === 'closed'
                    ...
                    return (
                      <li key={consultation.id}>
                        <Link
                          href={{ pathname: `/patient/${consultation.id}`, query: { userId: consultation.userId } }}
                            ...
                            <div className="min-w-0 space-y-2">
                              <div className="flex flex-wrap items-center gap-2">
                                <span className="text-sm font-semibold text-zinc-800">
                                  {consultation.patientData.age || '나이 미상'} / {genderLabel(consultation.patientData.gender)}
                                </span>
                                ...
                                {consultation.sourceLanguage ? (
                                  <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700">
                                    원문 {languageLabel(consultation.sourceLanguage)}
                                  </span>
                                ) : null}
                              </div>
                              <p className="truncate text-sm text-zinc-700">
                                <span className="mr-1 font-medium text-zinc-500">주호소</span>
                                {complaintForDoctor(consultation)}
                              </p>
                              {symptomForDoctor(consultation) ? (
                                <p className="line-clamp-2 text-sm text-zinc-500">{symptomForDoctor(consultation)}</p>
                              ) : null}
                              ...
                              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-400">
                                {consultation.patientData.nrs ? <span>NRS {consultation.patientData.nrs}</span> : null}
                                {consultation.patientData.associated ? <span>{consultation.patientData.associated}</span> : null}
                                <span>{secondaryLabel} {formatDateTime(secondaryTimestamp)}</span>
                              </div>
                            </div>
                            <div className="shrink-0 text-right">
                              <span className="block text-xs text-zinc-400">{formatElapsed(consultation.createdAt)}</span>
                              <span className="mt-1 block text-[11px] text-zinc-400">{consultation.id.slice(0, 8)}</span>
                            </div>
                          </div>
                        </Link>
                      </li>
                ...
                {totalPages > 1 ? (
                  <div className="flex items-center justify-between rounded-2xl border border-zinc-200 bg-white px-4 py-3 shadow-sm">
                    <p className="text-xs text-zinc-400">{currentPage} / {totalPages} 페이지</p>
                    <div className="flex items-center gap-2">
                      <button
